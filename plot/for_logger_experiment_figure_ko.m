...
load("log_ptop_LoadMassEst.mat");

eval('log = log_ptop_LoadMassEst;');%ここにもファイル名書かなきゃダメ
...
% Agent = LOGGER.Data.agent;
NNN=1;%for loop用（機体数分回すため
AgentNumber=length(log.target);
Agent(1) = log_ptop_LoadMassEst.Data.agent(1);
Agent(2)= log_ptop_LoadMassEst.Data.agent(2);
...
% initialize data
Agent(NNN).Est = zeros(12, flight_finish_idx-arming_start_idx+1);
Agent(NNN).Road_est = zeros(9, flight_finish_idx-arming_start_idx+1);
Agent(NNN).Road_sensor = zeros(3, flight_finish_idx-arming_start_idx+1);
Agent(NNN).Ref = zeros(3,  flight_finish_idx-arming_start_idx+1);
Agent(NNN).Input = zeros(4,flight_finish_idx-arming_start_idx+1);
Agent(NNN).InnerInput = zeros(8, flight_finish_idx-arming_start_idx+1);
...
for i = arming_start_idx:flight_finish_idx
    Agent(NNN).Est(:,i-arming_start_idx+1) = [Agent(NNN).estimator.result{i}.state.p;
                Agent(NNN).estimator.result{i}.state.q;
                Agent(NNN).estimator.result{i}.state.v;
                Agent(NNN).estimator.result{i}.state.w];
     Agent(NNN).Road_est(:,i-arming_start_idx+1) = [Agent(NNN).estimator.result{i}.state.pL;
                 Agent(NNN).estimator.result{i}.state.vL;
                 Agent(NNN).estimator.result{i}.state.wL];
     Agent(NNN).Load_sensor(:,i-arming_start_idx+1) = [Agent(NNN).sensor.result{i}.state.pL;%センサーの真値が必要な場合はこれを使う．
                % Agent.sensor.result{i}.state.vL;
                 %Agent.sensor.result{i}.state.wL
                 ];
    Agent(NNN).Ref(:,i-arming_start_idx+1) = [Agent(NNN).reference.result{i}.state.p];

    Agent(NNN).Input(:,i-arming_start_idx+1) = Agent(NNN).input{i};
% cul_step_time(i)=logt(1,i)-logt(1,i-1);
kari_logt=[0;logt(1:end-1)];
    Step_time= logt-kari_logt;
    
     InnerInput(:,i-arming_start_idx+1) = Agent(NNN).inner_input{i};
     if isfield(Agent(NNN).controller.result{flight_start_idx}, 'Z1')%Agent.controller.result{flight_start_idx}.Z1 ~= 0
z_ari=1;%zを求める場合は1になる．
    else 
    z_ari=0;
    end
      if z_ari ==1 && i >=flight_start_idx
      Z_1(:,i-arming_start_idx+1)=[Agent1.controller.result{i}.Z1];
      Z_2(:,i-arming_start_idx+1)=[Agent1.controller.result{i}.Z2];
      Z_3(:,i-arming_start_idx+1)=[Agent1.controller.result{i}.Z3];
      Z_4(:,i-arming_start_idx+1)=[Agent1.controller.result{i}.Z4];
      end
end
...
Agent(NNN).RMSE_x_flight_only = 0;
Agent(NNN).RMSE_y_flight_only = 0;
Agent(NNN).RMSE_z_flight_only = 0;

Agent(NNN).RMSE_x_flight_only_Load = 0;
Agent(NNN).RMSE_y_flight_only_Load = 0;
Agent(NNN).RMSE_z_flight_only_Load = 0;

for i = flight_start_idx:flight_finish_idx
    Agent(NNN).RMSE_x_flight_only = Agent(NNN).RMSE_x_flight_only + (Agent(NNN).Est(1, i-arming_start_idx+1) - Agent(NNN).Ref(1, i-arming_start_idx+1))^2;
    Agent(NNN).RMSE_y_flight_only = Agent(NNN).RMSE_y_flight_only + (Agent(NNN).Est(2, i-arming_start_idx+1) - Agent(NNN).Ref(2, i-arming_start_idx+1))^2;
    Agent(NNN).RMSE_z_flight_only = Agent(NNN).RMSE_z_flight_only + (Agent(NNN).Est(3, i-arming_start_idx+1) - Agent(NNN).Ref(3, i-arming_start_idx+1))^2;

    Agent(NNN).RMSE_x_flight_only_Load = Agent(NNN).RMSE_x_flight_only_Load + (Agent(NNN).Road_est(1, i-arming_start_idx+1) - Agent(NNN).Ref(1, i-arming_start_idx+1))^2;
    Agent(NNN).RMSE_y_flight_only_Load = Agent(NNN).RMSE_y_flight_only_Load + (Agent(NNN).Road_est(2, i-arming_start_idx+1) - Agent(NNN).Ref(2, i-arming_start_idx+1))^2;
    Agent(NNN).RMSE_z_flight_only_Load = Agent(NNN).RMSE_z_flight_only_Load + (Agent(NNN).Road_est(3, i-arming_start_idx+1) - Agent(NNN).Ref(3, i-arming_start_idx+1))^2;
end
num_samples_flight_only = flight_finish_idx - flight_start_idx + 1;
Agent(NNN).RMSE_x_flight_only = sqrt(Agent(NNN).RMSE_x_flight_only / num_samples_flight_only);
Agent(NNN).RMSE_y_flight_only = sqrt(Agent(NNN).RMSE_y_flight_only / num_samples_flight_only);
Agent(NNN).RMSE_z_flight_only = sqrt(Agent(NNN).RMSE_z_flight_only / num_samples_flight_only);

Agent(NNN).RMSE_x_flight_only_Load = sqrt(Agent(NNN).RMSE_x_flight_only_Load / num_samples_flight_only);
Agent(NNN).RMSE_y_flight_only_Load = sqrt(Agent(NNN).RMSE_y_flight_only_Load / num_samples_flight_only);
Agent(NNN).RMSE_z_flight_only_Load = sqrt(Agent(NNN).RMSE_z_flight_only_Load / num_samples_flight_only);
...
Agent(NNN).RMSE_x_after_A_B = 0;
Agent(NNN).RMSE_y_after_A_B = 0;
Agent(NNN).RMSE_z_after_A_B = 0;

for i = idx_A_seconds:idx_B_end
    Agent(NNN).RMSE_x_after_A_B = Agent(NNN).RMSE_x_after_A_B + (Agent(NNN).Road_est(1, i-arming_start_idx+1) - Agent(NNN).Ref(1, i-arming_start_idx+1))^2;
    Agent(NNN).RMSE_y_after_A_B = Agent(NNN).RMSE_y_after_A_B + (Agent(NNN).Road_est(2, i-arming_start_idx+1) - Agent(NNN).Ref(2, i-arming_start_idx+1))^2;
    Agent(NNN).RMSE_z_after_A_B = Agent(NNN).RMSE_z_after_A_B + (Agent(NNN).Road_est(3, i-arming_start_idx+1) - Agent(NNN).Ref(3, i-arming_start_idx+1))^2;
end
num_samples_after_A_B = idx_B_end - idx_A_seconds + 1;
Agent(NNN).RMSE_x_after_A_B = sqrt(Agent(NNN).RMSE_x_after_A_B / num_samples_after_A_B);
Agent(NNN).RMSE_y_after_A_B = sqrt(Agent(NNN).RMSE_y_after_A_B / num_samples_after_A_B);
Agent(NNN).RMSE_z_after_A_B = sqrt(Agent(NNN).RMSE_z_after_A_B / num_samples_after_A_B);

% Calculate RMSE for takeoff start + C seconds, D seconds period%複数機かは後回し
...
Agent(NNN).RMSE_x_after_C_D = 0;
Agent(NNN).RMSE_y_after_C_D = 0;
Agent(NNN).RMSE_z_after_C_D = 0;

for i = idx_C_seconds:idx_D_end
   Agent(NNN).RMSE_x_after_C_D = Agent(NNN).RMSE_x_after_C_D + (Agent(NNN).Est(1, i-arming_start_idx+1) - Agent(NNN).Ref(1, i-arming_start_idx+1))^2;
    Agent(NNN).RMSE_y_after_C_D = Agent(NNN).RMSE_y_after_C_D + (Agent(NNN).Est(2, i-arming_start_idx+1) - Agent(NNN).Ref(2, i-arming_start_idx+1))^2;
    Agent(NNN).RMSE_z_after_C_D = Agent(NNN).RMSE_z_after_C_D + (Agent(NNN).Est(3, i-arming_start_idx+1) - Agent(NNN).Ref(3, i-arming_start_idx+1))^2;
end
num_samples_after_C_D = idx_D_end - idx_C_seconds + 1;
Agent(NNN).RMSE_x_after_C_D = sqrt(Agent(NNN).RMSE_x_after_C_D / num_samples_after_C_D);
Agent(NNN).RMSE_y_after_C_D = sqrt(Agent(NNN).RMSE_y_after_C_D / num_samples_after_C_D);
Agent(NNN).RMSE_z_after_C_D = sqrt(Agent(NNN).RMSE_z_after_C_D / num_samples_after_C_D);

% Display results
fprintf('Flight-only RMSE for x direction: %.4f [m]\n', Agent(NNN).RMSE_x_flight_only);
fprintf('Flight-only RMSE for y direction: %.4f [m]\n', Agent(NNN).RMSE_y_flight_only);
fprintf('Flight-only RMSE for z direction: %.4f [m]\n', Agent(NNN).RMSE_z_flight_only);

fprintf('Flight-only_Load RMSE for x direction: %.4f [m]\n', Agent(NNN).RMSE_x_flight_only_Load);
fprintf('Flight-only_Load RMSE for y direction: %.4f [m]\n', Agent(NNN).RMSE_y_flight_only_Load);
fprintf('Flight-only_Load RMSE for z direction: %.4f [m]\n', Agent(NNN).RMSE_z_flight_only_Load);

fprintf('RMSE for x direction after %d seconds from flight start for %d seconds: %.4f [m]\n', A, B, Agent(NNN).RMSE_x_after_A_B);
fprintf('RMSE for y direction after %d seconds from flight start for %d seconds: %.4f [m]\n', A, B, Agent(NNN).RMSE_y_after_A_B);
fprintf('RMSE for z direction after %d seconds from flight start for %d seconds: %.4f [m]\n', A, B, Agent(NNN).RMSE_z_after_A_B);

fprintf('RMSE for x direction after %d seconds from takeoff for %d seconds: %.4f [m]\n', C, D, Agent(NNN).RMSE_x_after_C_D);
fprintf('RMSE for y direction after %d seconds from takeoff for %d seconds: %.4f [m]\n', C, D, Agent(NNN).RMSE_y_after_C_D);
fprintf('RMSE for z direction after %d seconds from takeoff for %d seconds: %.4f [m]\n', C, D, Agent(NNN).RMSE_z_after_C_D);
...
aaa=1;
    ...
    figure(aaa); plot(logt, Agent(NNN).Est(1:3,:)); hold on; plot(logt, Agent(NNN).Ref(1:3,:), '--'); hold off;
    ...
    % attitude
    figure(2); plot(logt, Agent(NNN).Est(4:6,:)); %hold on; plot(logt, Ref(4:6, :), '--'); hold off;
...
if z_ari==0
    m = 2; n = 3;
 plot(logt, Agent(NNN).Road_est(1:3,:), '--'); hold on; plot(logt, Agent(NNN).Est(1:3,:), '--');plot(logt, Agent(NNN).Ref(1:3,:)), hold off;
      subplot(m,n,1);  plot(logt, Agent(NNN).Road_est(1:3,:), '--'); hold on; plot(logt, Agent(NNN).Est(1:3,:), '--');plot(logt, Agent(NNN).Ref(1:3,:)), hold off;
     xlabel("Time [s]",'Interpreter','latex'); ylabel("Position [m]",'Interpreter','latex'); legend("$$x$$.Load state", "$$y$$.Load state", "$$z$$.Load state","$$x$$.Drone state", "$$y$$.Drone state", "$$z$$.Drone state", "$$x$$.Reference", "$$y$$.Reference", "$$z$$.Reference",  "Location","northwest",'Interpreter','latex');
      grid on; xlim([logt(1), logt(end)]); ylim([-inf inf]);
    % title("Time change of Position"); 
