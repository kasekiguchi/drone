...
function [allData,RMSElog]=dataSummarize2(loggers, c, option, addingContents, fF, startTime, endTime)
    ...
    if fF==1
        ...
        allData.xi = { struct('x',{time},'y',{cinput}), struct('x','time (s)','y','Imaginaly state'), LgndCrt(["T","roll","pitch","yaw"],c),add_option([],option,addingContents)};
...
function plot_data_single(~, ~, branchData)
    ...
    function newlog = changeResult(log,controllerName)
        ...
        for i = 1:newlog.k
            ...
            newlog.Data.agent.NN_xi{1, i} = log.Data.agent.controller.result{1, i}.(controllerName).NN_xi;
        end
