...
load("HC15_h1_No1_Log(07-Jan-2025_17_29_20)");
log2=log
load("HC15_h1_NO2_Log(07-Jan-2025_17_29_21)");
...
tflame=1458;
 arming_start_idx = find(log.Data.phase==102, 1, 'first')+tflame;%フライト開始からのグラフにできる．↓と切り替え
...
flight_start_idx = find(log.Data.phase==102, 1, 'first')+tflame;%フライト開始
...
% logt=logt-logt(1);%これやるとグラフが0秒からになる
...
%% %log2用
figtype = 1;%1でグラフを1タブづつ，2で1タブにグラフを多数．
Agent2 = log2.Data.agent;
tflame2=1458;
 arming_start_idx2 = find(log2.Data.phase==102, 1, 'first')+tflame;%フライト開始からのグラフにできる．↓と切り替え
% arming_start_idx = find(log.Data.phase==115, 1, 'first');%プログラム開始からのグラフ，↑と切り替え
takeoff_start_idx2 = find(log2.Data.phase==116, 1, 'first');%テイクオフ開始
flight_start_idx2 = find(log2.Data.phase==102, 1, 'first')+tflame;%フライト開始
takeoff_finish_idx2 = find(log2.Data.phase==116, 1, 'last');
flight_finish_idx2 = find(log2.Data.phase==102, 1, 'last');%)の後に-1000すれば1000フレーム前で切れる．ランディングおす1フレーム前
 logt2 = log2.Data.t(arming_start_idx2:flight_finish_idx2);
% logt=logt-logt(1);%これやるとグラフが0秒からになる

%logt=linspace(0,logt(end)-logt(1),flight_finish_idx-arming_start_idx+1);%ここコメントアウトしたら0sからじゃなくなる

% initialize data
Est2 = zeros(12, flight_finish_idx2-arming_start_idx2+1);
Road_est = zeros(9, flight_finish_idx-arming_start_idx+1);
Load_sensor = zeros(3, flight_finish_idx-arming_start_idx+1);
Ref2 = zeros(3,  flight_finish_idx2-arming_start_idx2+1);
Input2 = zeros(4,flight_finish_idx2-arming_start_idx2+1);
InnerInput2 = zeros(8, flight_finish_idx2-arming_start_idx2+1);
Step_time2=zeros(flight_finish_idx2-arming_start_idx2+1,1);%アーミングからフライト
Flight_step_time2=zeros(flight_finish_idx2-flight_start_idx2+1,1);%フライトステップのみ

% Z_1=zeros(2,flight_finish_idx-arming_start_idx+1);
% Z_2=zeros(6,flight_finish_idx-arming_start_idx+1);
% Z_3=zeros(6,flight_finish_idx-arming_start_idx+1);
% Z_4=zeros(2,flight_finish_idx-arming_start_idx+1);
for i = arming_start_idx2:flight_finish_idx2
    Est2(:,i-arming_start_idx2+1) = [Agent2.estimator.result{i}.state.p;
                Agent2.estimator.result{i}.state.q;
                Agent2.estimator.result{i}.state.v;
                Agent2.estimator.result{i}.state.w];
     % Road_est(:,i-arming_start_idx+1) = [Agent.estimator.result{i}.state.pL;
     %             Agent.estimator.result{i}.state.vL;
     %             Agent.estimator.result{i}.state.wL];
     % Load_sensor(:,i-arming_start_idx+1) = [Agent.sensor.result{i}.state.pL;%センサーの真値が必要な場合はこれを使う．
     ...
     %             %Agent.sensor.result{i}.state.wL
     %             ];
    Ref2(:,i-arming_start_idx2+1) = [Agent2.reference.result{i}.state.p];

    Input2(:,i-arming_start_idx2+1) = Agent2.input{i};
% cul_step_time(i)=logt(1,i)-logt(1,i-1);
kari_logt2=[0;logt2(1:end-1)];
    Step_time2= logt2-kari_logt2;
    
     InnerInput2(:,i-arming_start_idx2+1) = Agent2.inner_input{i};
%      if isfield(Agent.controller.result{flight_start_idx}, 'Z1')%Agent.controller.result{flight_start_idx}.Z1 ~= 0
% z_ari=1;%zを求める場合は1になる．
%     else 
%     z_ari=0;
...
count_gross_over_00252=length( find( Step_time2 >= 0.025 ) )
count_persentage_over_00252=length( find( Step_time2 >= 0.025 ) )/length(Step_time2)
% Flight_step_time=Step_time(flight_start_idx:flight_finish_idx);
Flight_step_time_average2=mean(Flight_step_time2)
...
flight_times = logt - logt(tflame); % arm=flightに書き換えたときのみコメントアウトtflameからの時間↑↓
...
%%
logt2=logt2-(logt2(1)-logt(1));
% m = 2; n = 3;
if figtype == 1
    % Title = strcat('LandingFreeFall', '-N', num2str(data.param.Maxparticle_num), '-', num2str(te), 's-', datestr(datetime('now'), 'HHMMSS'));
    figure(1); plot(logt, Est(1:3,:)); hold on; plot(logt, Ref(1:3,:), '--'); plot(logt2, Est2(1:3,:)); plot(logt2, Ref2(1:3,:), '--'); hold off;
    xlabel("Time [s]",'Interpreter','latex'); ylabel("Position [m]",'Interpreter','latex'); legend("$$x$$.Drone1", "$$y$$.Drone1", "$$z$$.Drone1", "$$x$$.Reference1", "$$y$$.Reference1", "$$z$$.Reference1", ...
        "$$x$$.Drone2", "$$y$$.Drone2", "$$z$$.Drone2", "$$x$$.Reference2", "$$y$$.Reference2", "$$z$$.Reference2",...
...
%    figure(5); plot(logt, InnerInput);legend("1","2", "3", "4","5","6", "7", "8",'Interpreter','latex');
    ...
    ytickformat('%.1f');
    %以下三宅整備中1/2
    figure(6); plot(Est(1,:), Est(2,:)); hold on; plot(Ref(1,:), Ref(2,:), '--'); plot(Est2(1,:), Est2(2,:));plot(Ref2(1,:), Ref2(2,:), '--'); hold off;
    grid on;xticks(-2.5:0.5:2.5);yticks(-2.5:0.5:2.5); xlim([-2.5, 2.5]); ylim([-2.5, 2.5]);pbaspect([1 1 1]);
    xlabel('$$x$$ [m]','Interpreter','latex'); ylabel("$$y$$ [m]",'Interpreter','latex'); legend("Drone", "Reference","Drone2", "Reference2",'Interpreter','latex');
    %ytickformat('%.1f');
    % Title = 
    %荷物のプロット↓
