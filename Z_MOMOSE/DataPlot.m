...
%         "Linear state FB10",...
%         "Finit time settling10",...
%         "Linear state FB30",...
%         "Finit time settling30",...
%         "Linear state FB50",...
%         "Finit time settling50",...
%         "Linear state FB100",...
%         "Finit time settling100",...
%         "Linear state FBmn10",...
%         "Finit time settling mn10",...
%         "Linear state FBmn30",...
%         "Finit time settlingmn30",...
%         "Linear state FBmn40",...
%         "Finit time settlingmn40",...
%         "Linear state FBm40",...
...
elseif fspider ==1
    ...
    figName = ["subsystem_x","angle_angleVel","angle_vel","err_RMSE","est","velocity","est_ver","subsystem_y","uHL","input","qw_RMSE","pv_RMSE"];%%
    %legend
    AL={{'$x$', '$dx$', '$ddx$', '$dddx$'},...
         ...
         {'$q_{roll}$ RMSE [rad]','$q_{pitch}$ RMSE [rad]','$q_{yaw}$ RMSE [rad]','$w_{roll}$ RMSE [rad/s]','$w_{pitch}$ RMSE [rad/s]','$w_{yaw}$ RMSE [rad/s]'},...
         {'$x$ RMSE [m]','$y$ RMSE [m]','$z$ RMSE [m]','$v_x$ RMSE [m/s]','$v_y$ RMSE [m/s]','$v_z$ RMSE [m/s]'}};
    legend_str = c;
...
subfolder='sim';%sim or exp
ExpSimName='モデル誤差';%実験名,シミュレーション名
% contents='FT_apx_max';%実験,シミュレーション内容
contents='km1_1';%実験,シミュレーション内容
%==========================================================
