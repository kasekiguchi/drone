classdef LocalPlanning_ControllerForODVADI <CONTROLLER_CLASS
    ...
    methods
        function obj = LocalPlanning_ControllerForODVADI(self,param)
            obj.self = self;
        ...
        function result = do(obj,param,~)
            ...
            LineParam = CFODVADI_ConvertLineParam(obj);
...
%             else
            ...
            problem.objective = @(x) LMPobjectiveForODVADI(obj,x, params);  % 評価関数
            problem.x0		  = oldinput;
...
%             if isfield(obj.self.model.state,'v')
                %加速度次元入力
                RobotVxyW = [obj.self.estimator.result.state.v(1),obj.self.estimator.result.state.v(2),obj.self.estimator.result.state.w];
                obj.result.input = [(Solvex(1) - RobotVxyW(1)) , (Solvex(2) - RobotVxyW(2)) , (Solvex(3) - RobotVxyW(3))];
...
%             end
    ...
    methods(Access = private)
        outStruct = CFODVADI_ConvertLineParam(obj);
        eval = LMPobjectiveForODVADI(obj,x, params);
    end
