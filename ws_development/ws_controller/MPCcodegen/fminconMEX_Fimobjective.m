...
% -------------------------------------------------------------------------
function [x,fval,exitflag,output,lambda,grad,hessian] = fminconMEX_Fimobjective(x0,param,ModelHandle)
...
assert(isa(param.dis,'double'));assert(all(size(param.dis)>=	[1,1]));assert(all(size(param.dis)<=	[629,1]));
assert(isa(param.alpha,'double'));assert(all(size(param.alpha)>=	[1,1]));assert(all(size(param.alpah)<=	[629,1]));
assert(isa(param.phi,'double'));assert(all(size(param.phi)>=	[1,1]));assert(all(size(param.phi)<=	[629,1]));
assert(isa(param.X0,'double'));assert(all(size(param.X0)==	[5,1]));
% assert(isa(param.model,'function_handle'));
...
assert(isa(param.model_param.K,'double'));assert(all(size(param.model_param.K)==	[1,1]));
    ...
    evalfunc = @(x)FimobjectiveMEX(x,param);
    nonlcon  = @(x)constraintsMEX(x,param,ModelHandle);
