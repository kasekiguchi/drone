...
Xm = Rx + Constant.SensorRange * cos(measured_angle).*(measured_angle~=0);
Ys = map.y(:, 1);
Ye = map.y(:, 2);   
Ry = state(2);
%Ym = Ry + Constant.SensorRange * sin(state(3) + measured_angle);
Ym = Ry + Constant.SensorRange * sin(measured_angle).*(measured_angle~=0);
...
min_dist2(del_ids) =0;
% del_ids2=min_dist2 > 15;
% min_index2(del_ids2)=0;
% min_dist2(del_ids2) =0;
% del_ids3=min_dist2 < 0.1;
% min_index2(del_ids3)=0;
% min_dist2(del_ids3) =0;
