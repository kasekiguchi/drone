function typical_Estimator_EKFSLAM_WheelChair(agent)

%% estimator class demo
% estimator property 繧脱stimator class縺ョ繧、繝ウ繧ケ繧ソ繝ウ繧ケ驟榊?励→縺励※螳夂セゥ
% 縺吶∋縺ヲ縺ョ讖滉ス薙〒蜷御ク?險ュ螳?
Estimator.name="ekfslam_WC";
Estimator.type="EKFSLAM_WheelChair";%propose method 
% Estimator.type="CmpEKFSLAM_WheelChair";%the compare method as 3 state

dt = agent(1).model.dt;
%    EKF_param.Q = eye(3)*7.058E-5;%.*[50;50;50;1E04;1E04;1E04];%1.0e-1; % 繧キ繧ケ繝?繝?繝弱う繧コ?シ?Model繧ッ繝ゥ繧ケ逕ア譚・?シ?
    EKF_param.Q = diag([1.0, 1.0, 1.0 10 10]);
    %     EKF_param.Q(3,3) = 0;
%     EKF_param.Q(6,6) = 0;%鬮倥＆譁ケ蜷代?ッ蛻?謨」縺ェ縺?
%     EKF_param.R = diag([[6.716E-5; 7.058E-5; 7.058E-5];[6.716E-5; 7.058E-5; 7.058E-5]/100]);%eye(6).*1.0E-4;%1.0e-1; % 隕ウ貂ャ繝弱う繧コ?シ?Sensor繧ッ繝ゥ繧ケ逕ア譚・?シ?
    EKF_param.R = 1.0E-3;
    EKF_param.Map_Q = eye(2).*1.0E-6;
    
%     n = length(EKF_param.Q);
    n = agent(1).model.dim(1);
%    EKF_param.JacobianH = @(~,~) eye(n); % C陦悟?? 蜈ィ迥カ諷玖ヲウ貂ャ
%     pqnum = sum(agent(1).model.state.num_list(contains(agent(1).model.state.list,["p","q"]))); % p, q 縺ョ谺。蜈?縺ョ蜥?
%     EKF_param.JacobianH = @(~,~) [eye(pqnum),zeros(pqnum,n-pqnum)];% C陦悟?? p, q隕ウ貂ャ
%     EKF_param.H = EKF_param.JacobianH;
%     EKF_param.B = [eye(6)*dt^2;eye(6)*dt];%eye(sum(agent(1).model.state.num_list)); % 繧キ繧ケ繝?繝?繝弱う繧コ縺悟刈繧上ｋ繝√Ε繝ウ繝阪Ν
    EKF_param.P = eye(n); % 蛻晄悄蜈ア蛻?謨」陦悟??
%     EKF_param.P(1,2) = 0.1;EKF_param.P(2,1) = 0.1;
    EKF_param.list=["p","v"];
Estimator.param=EKF_param;
for i = 1:length(agent)
    agent(i).set_estimator(Estimator);
end
disp('Execute "do" method with following parameter to operate EKF estimator.');
disp('param.estimator={{agent(i),agent(i).sensor.result.state.get(["q","p"]),[]}};')
end