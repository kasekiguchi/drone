classdef UKFSLAM_WheelChairA < ESTIMATOR_CLASS
    ...
    methods
        function obj = UKFSLAM_WheelChairA(self,param)
            obj.self= self;
            model = self.model;
            % --this state use in only UKFSLAM--
            obj.result.Est_state= [model.state.p;model.state.q;model.state.v;model.state.w];%x, y, theta,v,omega
            %------------------------------
            ...
            obj.constant.GroupNumberThreshold = 3; % Minimum points number which is constructed cluster
