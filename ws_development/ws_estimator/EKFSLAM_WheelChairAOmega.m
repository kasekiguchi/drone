classdef EKFSLAM_WheelChairAOmega < ESTIMATOR_CLASS
    ...
    methods
        function obj = EKFSLAM_WheelChairAOmega(self,param)
            obj.self= self;
            model = self.model;
...
%                     u(1) = u(1) * cparamK;
            ...
            pre_Eststate = xh_pre;
            A = eye(state_count);
            A(1:3,1:4) = [1,0,-sin(pre_Eststate(3))*pre_Eststate(4)*obj.self.model.dt,cos(pre_Eststate(3))*obj.self.model.dt;
                0,1,cos(pre_Eststate(3))*pre_Eststate(4)*obj.self.model.dt,sin(pre_Eststate(3))*obj.self.model.dt;
                0,0,1,0];
            ...
            for i = 1:association_available_count
                ...
                C(i, 4) = 0;%diff v
                C(i, 5 + (idx - 1) * 2) = 1 / cos(angle);%diff map param d
                C(i, 6 + (idx - 1) * 2) = (pre_state(1) * sin(line_param.delta(idx)) - pre_state(2) * cos(line_param.delta(idx))) / cos(angle) ...
            ...
            line_param.d = tmpvalue(5:2:end, 1);
            line_param.delta = tmpvalue(6:2:end, 1);
            ...
            if any(removing_flag)
                exist_flag = sort([1, 2, 3, 4,(find(~removing_flag) - 1) * 2 + 5, (find(~removing_flag) - 1) * 2 + 6]);
