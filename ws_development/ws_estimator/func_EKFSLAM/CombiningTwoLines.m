function parameter = CombiningTwoLines(map, index_map, measured, index_measured, Constant)
    ...
    dist = sqrt((Map1x - New1x) ^ 2 + (Map1y - New1y) ^ 2);
    Num = 1;
    distC = sqrt((Map1x - New2x) ^ 2 + (Map1y - New2y) ^ 2);
    if distC > dist
       dist = distC;
       Num = 2;
    end
    distC = sqrt((Map2x - New2x) ^ 2 + (Map2y - New2y) ^ 2);
    if distC > dist
       dist = distC;
       Num = 3;
    end
    distC = sqrt((Map2x - New1x) ^ 2 + (Map2y - New1y) ^ 2);
    if distC > dist
       dist = distC;
       Num = 4;
    end
    distC = sqrt((Map2x - Map1x) ^ 2 + (Map2y - Map1y) ^ 2);
    if distC > dist
       dist = distC;
       Num = 5;
    end
    distC = sqrt((New2x - New1x) ^ 2 + (New2y - New1y) ^ 2);
    if distC > dist
       Num = 6;
    end
