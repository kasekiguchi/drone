classdef EKFSLAM_ODVADI < ESTIMATOR_CLASS
    ...
    methods
        function obj = EKFSLAM_ODVADI(self,param)
            obj.self= self;
            model = self.model;
            %             obj.JacobianF = @(v,theta) [0,0,-v*sin(theta);0,0,v*cos(theta);0,0,0];%
            % --this state use in only EKFSLAM--
            obj.result.Est_state= STATE_CLASS(struct("state_list",["p","q","v","w"],"num_list",[2,1,2,1]));
            obj.result.Est_state.p = model.state.p;% x, y
            obj.result.Est_state.q = model.state.q;% th
            obj.result.Est_state.v = model.state.v;% v
            obj.result.Est_state.w = model.state.w;% W
            %------------------------------
...
%                 0,0,1,0,obj.self.model.dt];
            ...
            line_param.d = tmpvalue(obj.n+1:2:end, 1);
            line_param.delta = tmpvalue(obj.n+2:2:end, 1);
            % Convert line parameter into line equation "ax + by + c = 0"
            ...
            if any(removing_flag)
                exist_flag = sort([1, 2, 3, 4, 5, 6,(find(~removing_flag) - 1) * 2 + 7, (find(~removing_flag) - 1) * 2 + 8]);
