...
UKF_param.Q = diag([1e-2, 1e-2, 1e-2]);%*8.6;%ボットのシステムノイズ
% UKF_param.R = 3.5e-5;
UKF_param.R = 1.5e-6;%6.0e-5%3.6e-6;%1.0e-2%観測ノイズ1.4e-4
UKF_param.Map_Q =1.0e-3;%1.0e-5;
% UKF_param.Map_Q =1.0e-5;%1.0e-2%マップのシステムノイズ
n = agent.model.dim(1);%ロボGットモデルの状態数を呼び出し
UKF_param.dim = n;
...
UKF_param.constant.SegmentThreshold = 0.7;%0.5 線分からの距離：線分を直線に統合するかの基準
