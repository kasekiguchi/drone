function Estimator = Estimator_EKF_PE(agent,output,var)
    % output ：出力のリスト　例 ["p","q"]
    % var : 各出力に対するセンサーの観測ノイズs
    %% estimator class demo
    % estimator property をEstimator classのインスタンス配列として定義
    % すべての機体で同一設定
    Estimator.name="ekf_pe";
    Estimator.type="EKF_PE";
    dt = agent.model.dt;
...
%     end
%     if strcmp(agent.model.name,"load")
...
%         EKF_param.B = [[blkdiag([0.5*dt^2*eye(6);dt*eye(6)],[0.5*dt^2*eye(3);dt*eye(3)],[0.5*dt^2*eye(3);dt*eye(3)]);zeros(3,12)],[zeros(9,3);diag([1E4,1E4,1E4])*eye(3);zeros(9,3);diag([1E-4,1E-4,0]);1E-12*eye(3)]];
%     end
