function Model = Model_Suspended_Load_Extend(id,dt,type,initial,varargin)
% model class demo : quaternion model with 13 states
if ~isempty(varargin)
    Setting = varargin{1};
end
Model.type="Suspended_Load_Model_Extend"; % class name
Model.name="load_extend"; % print name
Setting.projection = @(x)[x(1:18);x(19:21)/norm(x(19:21));x(22:24)];
Setting.dim=[24,4,17];
Setting.input_channel = ["f","M"];
Setting.method = get_model_name("Load_Extend"); % model dynamicsの実体名
