...
%   詳細説明をここに記述
    ...
    Controller_param.Initsigma = 0.01;     %初期標準偏差
...
%     obj.param.QW = diag([10; 10; 10; 0.01; 0.01; 100.0]);  % 姿勢角、角速度
    ...
    %% 重みの設定
    Controller_param.P = diag([4.0; 2.0; 1.0]);    % 座標   1000 1000 100
    ...
    Controller_param.Q = diag([4000; 4000; 1500]);                % 姿勢角
    Controller_param.W = diag([1; 1; 1]);          % 角速度
    
    %終端での重みの設定
    Controller_param.Pf = diag([5.0; 2.0; 1.0]);
    Controller_param.Qf = diag([5000; 5000; 1500]);                
