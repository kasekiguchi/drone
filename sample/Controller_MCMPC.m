...
%     Controller_param.QW = diag([10; 10; 10; 0.01; 0.01; 100.0]);  % 姿勢角、角速度
    ...
    Controller_param.P = diag([500.0; 500.0; 200.0]);    % 座標   1000 1000 10000
    Controller_param.V = diag([50.0; 50.0; 100.0]);    % 速度
    Controller_param.R = diag([1.0; 1.0; 1.0; 1000.0]); % 入力
    Controller_param.RP = 0 * diag([1.0,; 1000.0; 1000.0; 1000.0]);  % 1ステップ前の入力との差    0*(無効化)
    Controller_param.QW = diag([100; 100; 100; 1; 1; 0]);  % 姿勢角、角速度
    ...
    Controller_param.CV = 0*10000; % 速度の係数
     ...
     Controller_param.Pf =   Controller_param.P ; % 6
    Controller_param.Vf =  Controller_param.V ; % 6
