...
%     Controller_param.P = diag([1e4; 1e4; 1e3]);    % 座標   1000 1000 10000
    Controller_param.P = diag([200; 200; 200]);    % 座標   1000 1000 10000
    Controller_param.V = diag([200; 200; 100]);    % 速度
    Controller_param.R = diag([1.0; 1; 1; 1]); % 入力
    Controller_param.RP = 0 * diag([1.0; 1; 1; 1]);  % 1ステップ前の入力との差    0*(無効化)
    Controller_param.Q = diag([100; 100; 200]);  % 姿勢角
    Controller_param.W = diag([100; 100; 100]);  % 角速度

    Controller_param.Pf = diag([100; 100; 200]); % 6
    Controller_param.Vf = diag([100; 100; 100]); % 6
    Controller_param.Qf = diag([100; 100; 100]); % 7,8
    Controller_param.Wf = diag([100; 100; 100]);
