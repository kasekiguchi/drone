...
%        EKF_param.B = [eye(6)*dt^2;eye(6)*dt]; % システムノイズが加わるチャンネル
    ...
    if strcmp(agent.model.name,"Suspended_Load_Model")
        EKF_param.Q = blkdiag(eye(3)*1E-3,eye(3)*1E-4,eye(3)*1E-3,eye(3)*1E-10);%[eye(3)*1E-3 zeros(3,9);zeros(3,3) eye(3)*1E-4 zeros(3,6);zeros(3,6) eye(3)*1E-3 zeros(3,3);zeros(3,9) eye(3)*1E-5];%eye(12)*1.5;%0.5E-1;%*7.058E-5;%diag(ones(n,1))*1e-7;%eye(6)*7.058E-5;%.*[50;50;50;1E04;1E04;1E04];%1.0e-1; % システムノイズ（Modelクラス由来）
        EKF_param.B = blkdiag([0.5*dt^2*eye(6);dt*eye(6)],[0.5*dt^2*eye(3);dt*eye(3)],[eye(3);eye(3)]);
