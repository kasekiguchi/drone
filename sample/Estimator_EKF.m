function Estimator = Estimator_EKF(agent,dt,model,output,opts)
    ...
    Estimator.output_value = @(self,param) self.sensor.result.state.get(param);
    Estimator.output_func = @(state,param) param*state;
    Estimator.sensor_param = ["p","q"];
    Estimator.output_param = Estimator.JacobianH(0,0);
