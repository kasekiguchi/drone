function Estimator = Estimator_EKF(agent,dt,model,output,opts)
    ...
    %%通常シミュレーション時
    Estimator.sensor_func = @(self,param) [self.sensor.result.state.get(param);self.sensor.lidar.result.length(1);self.sensor.lidar.result.length(3)] ; % function to get sensor value: sometimes some conversion will be done
