...
%        EKF_param.B = [eye(6)*dt^2;eye(6)*dt]; % システムノイズが加わるチャンネル
    ...
    if strcmp(agent.model.name,"load")
        EKF_param.Q = blkdiag(eye(3)*1E-4,eye(3)*8E-7,eye(3)*1E-4,eye(3)*1E-7); % システムノイズ（Modelクラス由来）
        EKF_param.B = blkdiag([0.5*dt^2*eye(6);dt*eye(6)],[0.5*dt^2*eye(3);dt*eye(3)],[0*eye(3);dt*eye(3)]);
    end
