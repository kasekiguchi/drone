function Controller = Controller_KPMCMPC(Agent, kp)
...
%   詳細説明をここに記述
    ...
    Controller_param.input.Maxsigma = 0.1 * [1.5;1.5;1.5;1.5];
    ...
    % Controller_param.W = diag([1e1; 1e1; 1e1]);  % 姿勢角、角速度
    % 
    % Controller_param.Pf = diag([1e2; 1e2; 1e4]); % 6
    % Controller_param.Vf = diag([1e2; 1e2; 1e3]); % 6
    % Controller_param.Qf = diag([1e1; 1e1; 1]);
    % Controller_param.Wf = diag([1; 1; 1]); % 7,8

    %% kiyama
    Controller_param.P = diag([40; 50; 30]);
    Controller_param.V = diag([5.0; 5.0; 35.0]);
    Controller_param.R = diag([10.0; 12.0; 5.0; 5.0]);
    Controller_param.RP = diag([0; 0; 0; 0]);
    Controller_param.Q = diag([500; 600; 500]);
    Controller_param.W = diag([7; 7; 15]);

    Controller_param.Pf = diag([1; 1; 1]);
    Controller_param.Vf = diag([1; 1; 1]);
    Controller_param.Qf = diag([1; 1; 1]);
    Controller_param.Wf = diag([1; 1; 1]);

    Controller_param.A = kp.est.A;
    Controller_param.B = kp.est.B;
    Controller_param.C = kp.est.C;

    Controller_param.input.u = Agent.parameter.mass*9.81/4 * [1;1;1;1];  % old version
    Controller_param.ref_input = Agent.parameter.mass*9.81/4 * [1;1;1;1];
    Controller_param.input.v = Controller_param.input.u;

    fprintf("KPMCMPC controller\n")

    Controller.name = "kpmpc";
