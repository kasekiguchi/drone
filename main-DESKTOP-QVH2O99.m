...
FieldSelect = {'straight','Like 6','Like H','TCU 2F','8widths straight','3widths straight'};
...
for i = 1:N
    ...
    if fExp
        ...
        agent(i) = Drone(Model_human_accelaration(i,N,dt,'plant',initial(i))); % unit quaternionのプラントモデル : for sim
...
for i=1:N
    while 1
        ...
        sponepoint = Cov_randspone(SponepointSetting(1),SponepointSetting(2),SponepointSetting(3),SponepointSetting(4),agent(i).env.huma_load.param.spone_area);
        TFin = isinterior(agent(i).env.huma_load.param.spone_area,sponepoint.x,sponepoint.y) ;%polyshapeの中に入っているかの判断
        tmp_dis = norm([sponepoint.x;sponepoint.y]-agent(i).plant.state.p(1:2),2);
        agent(i).plant.state.p(1:2)=[sponepoint.x;sponepoint.y];%人間を通路の端に配置
        R_name = agent(i).reference.name{1};
        Rxd = agent(i).reference.(R_name).xd;
        if norm(agent(i).plant.state.p(1:2)-Rxd,2)<5
            TFin =0;
        end
        if tmp_dis > 0.5
            if TFin==1
    ...
    if strcmp('straight',agent(i).env.huma_load.param.txt)
        ...
        elseif i==3
            agent(i).plant.state.p(1:2) = [1.5;3];
        elseif i==2
            agent(i).plant.state.p(1:2) = [3.;7];
        elseif i==4
...
while round(time.t,5)<=te
    ...
    for i = 1:N
        ...
        param(i).reference.slope={agent(i).sensor.result,agent(i).env,N,i,yasui,agent};
    ...
    yasui.dencity = Cov_CalclateDensity(agent,[1 15;1 16;9 16;4 15],yasui);
    [yasui,respone_flag] = yasui_SAVE_state_human(agent,yasui,respone_flag);
    yasui_fig_human(agent)
