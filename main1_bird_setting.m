...
if fMotive
    if fExp
        ...
        %% set connector (global instance)
        bird_ids = [1];
        motive_bird = Connector_Natnet('ClientIP', '192.168.1.9'); % Motive 7 : hara
        COMs = "COM21";
        ...
        bird_ids = 1:Nb;
        motive_bird = Connector_Natnet_sim(Nb, dt, 0);              % 3rd arg is a flag for noise (1 : active )
...
if Nb ~= 0
    initial_bird_state(Nb) = struct;
    param_bird(Nb) = struct('sensor', struct, 'estimator', struct, 'reference', struct);
end

if fExp
    if exist('motive', 'var') == 1; motive_bird.getData([], []); end

    for i = 1:Nb
        % for exp with motive : initial_stateize by motive info
        if exist('motive', 'var') == 1
            sstate = motive_bird.result.rigid(bird_ids(i));
            initial_bird_state(i).p = sstate.p;
            initial_bird_state(i).q = sstate.q;
            initial_bird_state(i).v = [0; 0; 0];
            initial_bird_state(i).w = [0; 0; 0];
        else % とりあえず用
            arranged_pos = arranged_position([0, 0], Nb, 1, 0);
    ...
    for i = 1:Nb
        ...
        if (fOffline)
            initial_bird_state(i).p = expdata.Data{1}.agent{1, expdata.si, i}.state.p;
            initial_bird_state(i).q = expdata.Data{1}.agent{1, expdata.si, i}.state.q;
            initial_bird_state(i).v = [0; 0; 0];
            initial_bird_state(i).w = [0; 0; 0];
        else
            arranged_pos = arranged_position([1, 1], Nb, 1, 0);
            initial_bird_state(i).p = arranged_pos(:, i);
            initial_bird_state(i).q = [1; 0; 0; 0];
            initial_bird_state(i).v = [0; 0; 0];
            initial_bird_state(i).w = [0; 0; 0];
