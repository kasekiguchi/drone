classdef Gemm_To_GemmLayer1031 < nnet.layer.Layer & nnet.layer.Formattable
    ...
    methods
        function this = Gemm_To_GemmLayer1031(name, onnxParams)
            this.Name = name;
            this.NumInputs = 2;
            this.OutputNames = {'x27'};
        ...
        function [x27] = predict(this, onnx__Gemm_0, onnx__Gemm_0NumDims)
            ...
            [x27, x27NumDims] = Gemm_To_GemmFcn(onnx__Gemm_0, onnx__Gemm_0NumDims, onnxParams, 'Training', false, ...
                'InputDataPermutation', {['as-is'], ['as-is']}, ...
                'OutputDataPermutation', {['as-is'], ['as-is']});
            if any(cellfun(@(A)~isnumeric(A) && ~islogical(A), {x27}))
                fprintf('Runtime error in network. At least one output of custom layer ''%s'' is a non-numeric, non-logical value.\n', 'Gemm_To_GemmLayer1031');
                error(message('nnet_cnn_onnx:onnx:BadCustomLayerRuntimeOutput', 'Gemm_To_GemmLayer1031'));
            end
            x27 = dlarray(single(x27), repmat('U', 1, max(2, x27NumDims)));
            if ~coder.target('MATLAB')
                x27 = extractdata(x27);
            end
        end

        function [x27] = forward(this, onnx__Gemm_0, onnx__Gemm_0NumDims)
            if isdlarray(onnx__Gemm_0)
                onnx__Gemm_0 = stripdims(onnx__Gemm_0);
            end
            onnx__Gemm_0NumDims = numel(onnx__Gemm_0NumDims);
            ...
            onnxParams.Learnables.linears_6_bias = this.linears_6_bias;
            onnxParams.Learnables.linears_6_weight = this.linears_6_weight;
            [x27, x27NumDims] = Gemm_To_GemmFcn(onnx__Gemm_0, onnx__Gemm_0NumDims, onnxParams, 'Training', true, ...
                'InputDataPermutation', {['as-is'], ['as-is']}, ...
                'OutputDataPermutation', {['as-is'], ['as-is']});
            if any(cellfun(@(A)~isnumeric(A) && ~islogical(A), {x27}))
                fprintf('Runtime error in network. At least one output of custom layer ''%s'' is a non-numeric, non-logical value.\n', 'Gemm_To_GemmLayer1031');
...
function [x27, x27NumDims, state] = Gemm_To_GemmFcn(onnx__Gemm_0, onnx__Gemm_0NumDims, params, varargin)
...
%
% [X27] = Gemm_To_GemmFcn(ONNX__GEMM_0, PARAMS)
...
%			network output(s) in X27.
%
% [X27, STATE] = Gemm_To_GemmFcn(ONNX__GEMM_0, PARAMS)
...
[x27, x27NumDims, state] = Gemm_To_GemmGraph1014(onnx__Gemm_0, NumDims.onnx__Gemm_0, Vars, NumDims, Training, params.State);
% Postprocess the output data
[x27] = postprocessOutput(x27, outputDataPerms, anyDlarrayInputs, Training, varargin{:});
end

function [x27, x27NumDims1030, state] = Gemm_To_GemmGraph1014(onnx__Gemm_0, onnx__Gemm_0NumDims1029, Vars, NumDims, Training, state)
% Function implementing the graph 'Gemm_To_GemmGraph1014'
% Update Vars and NumDims from the graph's formal input parameters. Note that state variables are already in Vars.
Vars.onnx__Gemm_0 = onnx__Gemm_0;
NumDims.onnx__Gemm_0 = onnx__Gemm_0NumDims1029;

% Execute the operators:
% Gemm:
[A, B, C, alpha, beta, NumDims.x_linears_0_Gemm_out] = prepareGemmArgs(Vars.onnx__Gemm_0, Vars.linears_0_weight, Vars.linears_0_bias, Vars.Gemmalpha1015, Vars.Gemmbeta1016, 0, 1, NumDims.linears_0_bias);
...
% Gemm:
[A, B, C, alpha, beta, NumDims.x_linears_1_Gemm_out] = prepareGemmArgs(Vars.x_activation_Tanh_ou, Vars.linears_1_weight, Vars.linears_1_bias, Vars.Gemmalpha1017, Vars.Gemmbeta1018, 0, 1, NumDims.linears_1_bias);
...
% Gemm:
[A, B, C, alpha, beta, NumDims.x_linears_2_Gemm_out] = prepareGemmArgs(Vars.x_activation_1_Tanh_, Vars.linears_2_weight, Vars.linears_2_bias, Vars.Gemmalpha1019, Vars.Gemmbeta1020, 0, 1, NumDims.linears_2_bias);
...
% Gemm:
[A, B, C, alpha, beta, NumDims.x_linears_3_Gemm_out] = prepareGemmArgs(Vars.x_activation_2_Tanh_, Vars.linears_3_weight, Vars.linears_3_bias, Vars.Gemmalpha1021, Vars.Gemmbeta1022, 0, 1, NumDims.linears_3_bias);
...
% Gemm:
[A, B, C, alpha, beta, NumDims.x_linears_4_Gemm_out] = prepareGemmArgs(Vars.x_activation_3_Tanh_, Vars.linears_4_weight, Vars.linears_4_bias, Vars.Gemmalpha1023, Vars.Gemmbeta1024, 0, 1, NumDims.linears_4_bias);
...
% Gemm:
[A, B, C, alpha, beta, NumDims.x_linears_5_Gemm_out] = prepareGemmArgs(Vars.x_activation_4_Tanh_, Vars.linears_5_weight, Vars.linears_5_bias, Vars.Gemmalpha1025, Vars.Gemmbeta1026, 0, 1, NumDims.linears_5_bias);
...
% Gemm:
[A, B, C, alpha, beta, NumDims.x27] = prepareGemmArgs(Vars.x_activation_5_Tanh_, Vars.linears_6_weight, Vars.linears_6_bias, Vars.Gemmalpha1027, Vars.Gemmbeta1028, 0, 1, NumDims.linears_6_bias);
Vars.x27 = alpha*B*A + beta*C;

% Set graph output arguments from Vars and NumDims:
x27 = Vars.x27;
x27NumDims1030 = NumDims.x27;
...
function [x27] = postprocessOutput(x27, outputDataPerms, anyDlarrayInputs, Training, varargin)
% Set output type:
...
x27 = permuteOutputVar(x27, outputDataPerms{1}, 2);
end
