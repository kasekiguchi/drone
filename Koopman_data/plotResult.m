...
loadfilename{5} = 'EstimationResult_12state_11_29_GUIsimdata.mat' ;%mainで書き込んだファイルの名前に逐次変更する
loadfilename{1} = 'case1.mat';
loadfilename{2} = 'case2.mat';
loadfilename{3} = 'case3.mat';
loadfilename{4} = 'case4.mat';
...
%% 各データセットの誤差グラフ
case1 = abs(file{1}.simResult.reference.est.p(1:stepN, :)' - file{1}.simResult.state.p(:, 1:stepN));
case2 = abs(file{2}.simResult.reference.est.p(1:stepN, :)' - file{2}.simResult.state.p(:, 1:stepN));
case3 = abs(file{3}.simResult.reference.est.p(1:stepN, :)' - file{3}.simResult.state.p(:, 1:stepN));
case4 = abs(file{4}.simResult.reference.est.p(1:stepN, :)' - file{4}.simResult.state.p(:, 1:stepN));
caset = 0:stepN-1;
figure(101); plot(caset, case1);
