function [FigNum] = PlotFunc_AttitudesTureAndEst(obj,FigNum)
labelstr = ["theta",];
%plant(true)
[~,PlantDim,PlantData,Flag] = FindDataMatchName(obj.logger,'plant.state.q');
if Flag
%estimation
[~,EstDim,EstData] = FindDataMatchName(obj.logger,'estimator.result.state.q');
if ~PlantDim ==EstDim
...
FigNum = FigNum + 1;
    ...
    disp(strcat('we do not disp AttitudesTureAndEst'));
end
end