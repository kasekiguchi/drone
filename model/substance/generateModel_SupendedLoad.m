...
ddpl  = [0;0;-gravity]+(dot(pT,u1*Rb0*e3)-m*Length*dot(dpT,dpT))*pT/(m+mL); % 牽引物体の加速度
ddp  = ddpl-Length*ddpT;
dob = inv(Ib)*cross(-ob,Ib*ob)+inv(Ib)*[u2;u3;u4]+inv(Ib)*cross(Rb0*[ex;ey;-ez],-mL*(ddpl+[0;0;gravity]));
dq   = L'*ob/2;
x=[q;ob;pl;dpl;pT;ol];
f=[dq;dob;dpl;ddpl;dpT;dol];
U = [u1;u2;u3;u4];
...
dob = inv(Ib)*cross(-ob,Ib*ob)+inv(Ib)*[u2;u3;u4]+inv(Ib)*cross(ERb0*[ex;ey;-ez],-mL*(ddpl+[0;0;gravity]));
dq   = EL'*ob/2;
x=[p;er;dp;ob;pl;dpl;pT;ol;ex;ey;ez];
...
syms Length real
syms ex ey ez real
physicalParam = {m, l, jx, jy, jz, gravity, km1, km2, km3, km4, k1, k2, k3, k4, mL, Length, ez};
e3=[0;0;1];
e1=[1;0;0];
dol  = cross(-pT,u1*ERb0*e3)/(m*Length); % ケーブル角加速度
dpT  = cross(ol,pT); % 機体から見たケーブル上の単位長さの位置の速度
...
dob = inv(Ib)*cross(-ob,Ib*ob)+inv(Ib)*[u2;u3;u4]+inv(Ib)*cross(ERb0*[ex;ey;-ez],-mL*(ddpl+[0;0;gravity]));
dq   = EL'*ob/2;
x=[p;er;dp;ob;pl;dpl;pT;ol;ex;ey];
