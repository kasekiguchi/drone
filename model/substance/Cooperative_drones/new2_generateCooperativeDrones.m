...
Mt = m0 + sum(mi);
J0b = J0;
for i = 1:N, J0b = J0b -mi(i)*Rho{i}^2; end
%%
dr0 = L0'*o0/2;
for i = 1:N
% (1)
...
%B3 = zeros(3,N);
for i = 1:N
  A12 = A12 - mi(i)*R0*Rho{i};
  A13{i} = mi(i)*li(i)*Qi{i};
  B1 = B1 - mi(i)*R0*O0^2*rho(:,i) - mi(i)*li(i)*(wi(:,i)'*wi(:,i))*qi(:,i);
  A21 = A21 + mi(i)*Rho{i}*R0';
  A23{i} = mi(i)*Rho{i}*R0'*li(i)*Qi{i};
  B2 = B2 - mi(i)*Rho{i}*R0'*li(i)*(wi(:,i)'*wi(:,i))*qi(:,i) + Rho{i}*R0'*(ui(:,i)+mi(i)*g*e3);
  A31{i} = -mi(i)*Qi{i};
  A32{i} = mi(i)*Qi{i}*R0*Rho{i};
  A33{i} = mi(i)*li(i)*eye(3);
  B3{i} = mi(i)*Qi{i}*R0*O0^2*rho(:,i) - Qi{i}*(ui(:,i)+mi(i)*g*e3);
end
%%
A = [A11 A12 [A13{:}];A21 A22 [A23{:}];vertcat(A31{:}) vertcat(A32{:}) blkdiag(A33{:})];
B = [B1;B2;reshape(B3,[],1)];
%%
syms iA [6+3*N 6+3*N] real
...
matlabFunction([dx0;dr0;ddX0(1:6);dqi;ddX0(7:end);dri;reshape(doi,[],1)],"File",dir+"tmp_cable_suspended_rigid_body_with_"+N+"_drones","Vars",{x R0 Ri u physicalParam ddX0},'outputs',{'dX'})
