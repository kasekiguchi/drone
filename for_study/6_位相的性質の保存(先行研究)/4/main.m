...
% plot(net_alpha)
%% 学習オプション
numEpochs = 1000; % 最大エポック
initialLearnRate = 0.01; % 初期学習率
decay = 0.01; % 減衰
momentum = 0.09; % モーメンタム
%% モニター
monitor = trainingProgressMonitor(Metrics="Loss",Info=["Epoch","LearnRate",'Loss'],XLabel="Epoch");
%% モデルの学習ξ
velocity = []; % パラメータ速度
epoch = 0; % エポック初期化
...
while epoch < numEpochs && ~monitor.Stop
    ...
    iteration = epoch;
    [loss,gradients,state] = dlfeval(@modelLoss,net_xi,X,T);
    net_xi.State = state;
    % 学習率更新
    ...
    updateInfo(monitor,Epoch=epoch,LearnRate=learnRate,Loss=loss);
...
output = modelPredictions(net_xi,XTest);
%%
figure()
hold on
for i = 1:28
    plot(output(1,:,i),output(2,:,i),'b')
end
grid on
xlim([-2 2])
ylim([-2 2])
