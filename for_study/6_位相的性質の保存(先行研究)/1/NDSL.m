classdef NDSL < handle
    %NDSL 先行研究の検証に用いるクラスファイル
    %   「軌道の位相的性質を保証する非線形動的システム学習」
    %   "Nonlinear Dynamical System Learning with
    %    Guaranteed Topological Property of Trajectories"
    %   事前学習を採用
    ...
    properties(Constant)
        inputSize1 = 2; % 入力層ユニット数
        outputSize1 = 2; % 出力層ユニット数
        inputSize2 = 2; % 入力層ユニット数
        outputSize2 = 2; % 出力層ユニット数
...
%             aenet.trainParam.epochs = 1000; % あまりよくない
            ae = train(aenet,x,x);
        end
