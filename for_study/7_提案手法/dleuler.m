function out = dleuler(Axy,Az,xi,Bxy,Bz,v)
...
for i = 1:vecs % 状態ベクトル毎に計算
    % ============
    % A * xi
    % ============
    % Axi = zeros(state,1);
    xi_x = [xi(3,i);xi(4,i);xi(5,i);xi(6,i)]; % xi_x
    xi_y = [xi(7,i);xi(8,i);xi(9,i);xi(10,i)]; % xi_y
    xi_z = [xi(1,i);xi(2,i)]; % xi_z
    xi_yaw = [xi(11,i);xi(12,i)]; % xi_yaw
    % x, y
    % for j = 1:rowA
    %     term1 = 0;
    %     term2 = 0;
    ...
    Axi_x = dlarray_prod_same_column(Axy,xi_xT);
    Axi_y = dlarray_prod_same_column(Axy,xi_yT);
    % Axi = dlarray_prod_same_row(Axy,xi_ano');
    Axi_z = dlarray_prod_same_column(Az,xi_zT);
    Axi_yaw = dlarray_prod_same_column(Az,xi_yawT);
    ...
    out(:,i) = [xikk_z;xikk_x;xikk_y;xikk_yaw];
