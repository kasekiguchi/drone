...
for n_data = 5:10
    ...
    filename = [rootname, num2str(n_data), nn.extension];
    load(filename);
    net1 = train(net1,x_train, zt_train);
    net2 = train(net2,z_train, xt_train);
%     date.n_num2str(n_date) = char(datetime('now','TimeZone','local','Format','yyyy-MM-dd HH:mm:ss'));
    fprintf('n=%d 学習完了\n',n_data);
end
date.FinishLearning = char(datetime('now','TimeZone','local','Format','yyyy-MM-dd HH:mm:ss'));
fprintf('%s 全ての学習が完了\n',date.FinishLearning);
%% 学習時間計測終了
fprintf('学習する際の');
toc
