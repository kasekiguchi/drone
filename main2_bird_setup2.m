...
%     if fExp
%         bird(i) = BIRD(Model_Drone_Exp(dt,initial_bird_state(i),"udp",[50,132]),BIRD_PARAM("TCUbird")); % for exp % 鳥の番号
%         bird(i).input = [0; 0; 0; 0];
%     else
% %         bird(i) = BIRD(Model_Bird(dt,initial_bird_state(i), i),BIRD_PARAM("TCUbird"));                % 離散時間のプラントモデル : for sim
%         bird(i) = BIRD(Model_Bird_EulerAngle(dt,initial_bird_state(i), i),BIRD_PARAM("TCUbird"));                % euler angleのプラントモデル : for sim
%     end

    %% model
    % set control model
%     bird(i).set_model(Model_Bird(dt,initial_bird_state(i),i),BIRD_PARAM("TCUbird")) % 離散時間モデル
    bird(i).set_model(Model_Bird_EulerAngle(dt,initial_bird_state(i),i),BIRD_PARAM("TCUbird")) % オイラー角モデル
    close all
...
%     end
    ...
    bird(i).set_property("sensor",Sensor_Map_Bird_3D(agent,'d',10)); % 測定距離d（第二引数宇）内の重要度を計測 : sim のみ 
