...
%             end % landing: ON
        ...
        %-- 加速度算出
            accT = dt;
            accZ = previous_state(3) - agent.model.state.p(3); 
            acc = accZ / accT;
        %-- データ保存
...
figure(9); plot(logger.Data.t, data.acc, 'LineWidth', 2); xlim([0 4]); xlabel("Time[s]"); ylabel("acceleration [m/s^2]"); set(gca,'FontSize',Fontsize); grid on;
%% animation
