...
log = LOGGER('./Data/PDAF_gooddata.mat');
O_func = @(x,u) On3(x,u);
...
%% RMSE
midt = 1600;
RMSE_P=sqrt(mse(robot_p(:,1:midt),robot_pe(:,1:midt)));
RMSE_Q=sqrt(mse(robot_q(:,1:midt),robot_qe(:,1:midt)));
RMSE_V=sqrt(mse(robot_v(:,1:midt),robot_ve(:,1:midt)));
RMSE_W=sqrt(mse(robot_w(:,1:midt),robot_we(:,1:midt)));
RMSE_blue = [RMSE_P;RMSE_Q;RMSE_V;RMSE_W]
RMSE_P=sqrt(mse(robot_p(:,midt:end),robot_pe(:,midt:end)));
RMSE_Q=sqrt(mse(robot_q(:,midt:end),robot_qe(:,midt:end)));
RMSE_V=sqrt(mse(robot_v(:,midt:end),robot_ve(:,midt:end)));
RMSE_W=sqrt(mse(robot_w(:,midt:end),robot_we(:,midt:end)));
RMSE_red = [RMSE_P;RMSE_Q;RMSE_V;RMSE_W]
RMSE_stack = zeros(4,length(robot_w));
for i=1:length(robot_p)
    RMSE_stack(1,i)=sqrt(mse(robot_p(:,1:i),robot_pe(:,1:i)));
    RMSE_stack(2,i)=sqrt(mse(robot_q(:,1:i),robot_qe(:,1:i)));
    RMSE_stack(3,i)=sqrt(mse(robot_v(:,1:i),robot_ve(:,1:i)));
    RMSE_stack(4,i)=sqrt(mse(robot_w(:,1:i),robot_we(:,1:i)));
end
fig33 = figure(33);
plot(time,RMSE_stack(1,:),'LineWidth', 3);
%% 
...
%%
 fig9=figure(9);
    ...
    plot(time,0.01*ones(size(time)),'LineWidth', 3);
    plot(time,0.01*ones(size(time)),'LineWidth', 3);
    patch([0 10 10 0], [-10 -10 20 20], [0.73 0.83 0.95], 'EdgeColor', 'none', 'FaceAlpha', 0.3);
    patch([10 20 20 10], [-10 -10 20 20], [1 0.8 0.8], 'EdgeColor', 'none', 'FaceAlpha', 0.3);
...
%     ylim([-1, 1]);
    ...
    patch([0 10 10 0], [-1 -1 2 2], [0.73 0.83 0.95], 'EdgeColor', 'none', 'FaceAlpha', 0.3);
    patch([10 20 20 10], [-1 -1 2 2], [1 0.8 0.8], 'EdgeColor', 'none', 'FaceAlpha', 0.3);
    xlim([0, maxt]);
           grid on;
%     ylim([-1 2]);
...
if illustration == 1
    ...
    plot(time,robot_qe(3,:),'LineWidth', 2);
    patch([0 10 10 0], [-1 -1 2 2], [0.73 0.83 0.95], 'EdgeColor', 'none', 'FaceAlpha', 0.3);
    patch([10 20 20 10], [-0.6 -0.6 0.6 0.6], [1 0.8 0.8], 'EdgeColor', 'none', 'FaceAlpha', 0.3);
