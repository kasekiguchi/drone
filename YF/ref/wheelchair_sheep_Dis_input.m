classdef wheelchair_sheep_Dis_input < REFERENCE_CLASS
    ...
    methods
        function obj = wheelchair_sheep_Dis_input(varargin)
...
%             rs = Param{5}(1);ra = Param{5}(2);
            rc = Param{5}(3); 
            tarm1 = zeros(2,length(other_state));
            adjust =  zeros(2,length(other_state));
            keep = zeros(2,1);
...
%                k5 = 0;%*distance{N}.result/(distance{N}.result + A.result);%%畑へ向かう力
            ...
            if count >1&&norm(agent_v{num})~=0
                ...
                obj.result.tmp1 = [k1*sum(tarm1,2)/Nb;k2*sum(adjust,2)/Nb;k3*keep;k4*sum(away,2)];
...
%             end
            kind_speed = 82 * 10/36;
            ref_point = eye(2)*state + eye(2)*(kind_speed * (input/norm(input,2)));
            th_xd = atan2(ref_point(2)-state(2),ref_point(1)-state(1));
%             figure;
...
%             ylim([state(2)-3-norm(ref_point-state),state(2)+3+norm(ref_point-state)])
            ...
            v_xd = kind_speed ;
            obj.result.state.u = 0*[v_xd;0;th_dd];%
            result = obj.result;
