...
rotms = matlabFunction([R;zero13;dR;zero13;d2R;zero13;d3R;zero13;d4R;zero13;d5R;zero13],'vars',t);%アルゴリズム上各行列ごとに0を入れている
% dixd = [xd;dxd;d2xd;d3xd;d4xd;d5xd];
% redixd = reshape(dixd,6,[]);
% ref = matlabFunction(reshape([redixd(1:3,:);redixd(6,:)],[],1),'vars',t);
% 
% rotms = eul2rotm(redixd(4:6,:)');
% rotms(4,:,:) = zeros(1,3,6);
% obj.result.rotms = reshape(pagetranspose(rotms),3,[],1)';%回転行列と高次微分
