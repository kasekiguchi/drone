classdef TIME_VARYING_REFERENCE_SUSPENDEDLOAD < REFERENCE_CLASS
    ...
    methods
        function obj = TIME_VARYING_REFERENCE_SUSPENDEDLOAD(self,varargin)
            ...
            if length(varargin{1})>2
                if strcmp(varargin{1}{3},"HL")
                    ...
                    obj.result.state=STATE_CLASS(struct('state_list',["xd","pL","p","yaw"],'num_list',[20,3,1]));
                end
            else
                obj.result.state=STATE_CLASS(struct('state_list',["xd","pL","p","yaw"],'num_list',[length(obj.func(0)),3,1]));
            end
        end
        ...
        function show(obj,logger)
            rp = logger.data(1,"pL","r");
            plot3(rp(:,1), rp(:,2), rp(:,3));                     % xy平面の軌道を描く
            daspect([1 1 1]);
            hold on
            ep = logger.data(1,"state.pL","e");
