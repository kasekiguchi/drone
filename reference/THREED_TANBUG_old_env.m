classdef THREED_TANBUG_old_env < REFERENCE_CLASS
    ...
    methods
        function obj = THREED_TANBUG_old_env(self, varargin)
            %obj.state = [0,0,0];
...
%            obj.path_goal = zeros(size(self.sensor.lrf.angle_range));
            ...
            hx= radius;
            hy = 0.1;%3D_enviroment_hv3(v)
            hz= 0.1;%3D_enviroment_hv3(v)

            P = [-0.01,hy,hz;-0.01,-hy,hz;-0.01,-hy,-hz;-0.01,hy,-hz;
...
%            obj.path_generator.show("FH",figure());
         ...
         function [g,v] = T_bug(~,obj,length,l_points,goal_length,id)%障害物の端点を算出
            %length:センサの距離データ
            %l_points:センサの座標データ
