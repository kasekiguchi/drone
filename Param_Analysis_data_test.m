...
%% Log Dataの読み取りと格納
log = readtable('20230719.csv');

%% ログ
tspan = [0 ,100];
% tspan = [0 99];
robot_p  = [log.Xhat0,log.Xhat1,log.Xhat2]';
robot_v  = [log.Xhat6,log.Xhat7,log.Xhat8]';
robot_q  = [log.Xhat3,log.Xhat4,log.Xhat5]'-[0;0;pi/2];
sensor_data = [log.scan1Y0,log.scan2Y0,log.scan3Y0,log.scan4Y0]';
...
for i=2:last-1
    ...
    qt = qt./vecnorm(qt,2);
    A = Cf2_sens(robot_p(:,i),qt,sensor_data(1,i+1),sensor_data(num,i+1),Rodrigues([0,0,1],snum(num)*pi/360));
    ds = find(A(2,:)==0);
    A(:,ds)=[];
    A_stack=[A_stack;A];
end
