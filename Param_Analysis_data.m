...
%% Log Dataの読み取りと格納
log = readtable("scan_data");
%% ログ
robot_p  = [log.Xhat0,log.Xhat1,log.Xhat2]';
robot_q  = [log.Xhat3,log.Xhat4,log.Xhat5]';
sensor_data = [log.scanY0,log.scanY1,log.scanY2]';
...
%% 解析(回帰分析)
[A,X,At,Xt] = param_analysis(robot_p(:,1:end),robot_q(:,1:end),sensor_data(1,:),eye(3));
[A2,X2,At2,Xt2] = param_analysis(robot_p(:,1:end),robot_q(:,1:end),sensor_data(2,:), Rodrigues([0,0,1],pi/2));
[A3,X3,At3,Xt3] = param_analysis(robot_p(:,1:end),robot_q(:,1:end),sensor_data(2,:), Rodrigues([0,0,2],pi/2));
%不要ぱらの除去
ds = find(A2(1,:)==0);
A2(:,ds) = [];
A(:,ds) = [];
