...
% agent.marge.drone = HLC(agent,Controller_HL(dt));
% agent.marge.load = HLC_SUSPENDED_LOAD(agent,Controller_HL_Suspended_Load(dt));
...
%    agent(1).marge.drone.do(time, 'f');
%    agent(1).marge.load.do(time, 'f');
