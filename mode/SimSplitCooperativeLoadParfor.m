...
for j = 1:tn
    ...
    agent(1).controller.do(time, 'f',0,0,agent(1),1);
    %複数機モデルから機体と接続点の位置を計測
    sensor1 = agent(1).sensor.result.state;
        %分割前ペイロード
        sp = sensor1.p;
    ...
    %推定の真値
     ...
     %ブロードキャスト変数の設定
     ppCagent1 = parallel.pool.Constant(agent(1));
     ppCagent1parameterli = parallel.pool.Constant(agent(1).parameter.li);
     ppCsensor1qi = parallel.pool.Constant(sensor1.qi);
     ppCsensor1Qi = parallel.pool.Constant(sensor1.Qi);
     ppCloadv = parallel.pool.Constant(load.v);
     ppCloadqi = parallel.pool.Constant(load.qi);
     ppCloadwi = parallel.pool.Constant(load.wi);
     ppCloadQi = parallel.pool.Constant(load.Qi);
     ppCloadOi = parallel.pool.Constant(load.Oi);
     % ppCsensor1Qi = parallel.pool.Constant(sensor1.Qi);
     % ppCsensor1Qi = parallel.pool.Constant(sensor1.Qi);
    % parfor i = 2:N+1
    for i = 2:N+1
            ...
            % 単機牽引のモデルで推定する
            agent(i).sensor.result.state.set_state("p",sp + sR * rho(:,i-1) - ppCagent1parameterli.Value(i-1)*ppCsensor1qi.Value(3*i-5:3*i-3,1), ...
                                                    "q",Quat2Eul(ppCsensor1Qi.Value(4*i-7:4*i-4,1)), ...
                                                    "pL",sp + sR * rho(:,i-1), ...
                                                    "pT",ppCsensor1qi.Value(3*i-5:3*i-3,1));
    ...
    for i = 2:N+1
        %単機牽引の真値
            agent(i).plant.state.set_state("pL",p_load + R_load * rho(:,i-1), ...
                "vL",ppCloadv.Value + dR_load * rho(:,i-1), ...
                "pT",ppCloadqi.Value(3*i-5:3*i-3,1), ...
                "wL",ppCloadwi.Value(3*i-5:3*i-3,1), ...
                "p",p_load + R_load * rho(:,i-1) - ppCagent1parameterli.Value(i-1)* ppCloadqi.Value(3*i-5:3*i-3,1), ...
                "v",ppCloadv.Value + dR_load * rho(:,i-1)- ppCagent1parameterli.Value(i-1)*Skew(ppCloadwi.Value(3*i-5:3*i-3,1))*ppCloadqi.Value(3*i-5:3*i-3,1), ...
                "q",Quat2Eul(ppCloadQi.Value(4*i-7:4*i-4,1)), ...
                "w",ppCloadOi.Value(3*i-5:3*i-3,1));
            % agent(i).reference.do(time, 'f',ppCagent1.Value); 
    end
    parfor i = 2:N+1
    % for i = 2:N+1
        agent(i).reference.do(time, 'f',ppCagent1.Value); 
