...
for check_i = 1:check_rigid
    x = agent(1).sensor.motive.result.rigid(check_i).p(1);
    y = agent(1).sensor.motive.result.rigid(check_i).p(2);
    ...
    elseif mod(check_i, 2) == 0
        % check_iが偶数のとき四角で表示(ドローンのはず
        plot(x, y, 's', 'MarkerSize', 10, 'DisplayName', ['Point ' num2str(check_i)]);
        text(x + 0.2, y, num2str(check_i), 'FontSize', 12, 'Color', 'black'); % 番号を表示
        
        if check_i + 1 <= check_rigid
            % 次の奇数の座標へ矢印を表示(単機牽引のロープの方向
            next_x = agent(1).sensor.motive.result.rigid(check_i + 1).p(1);
            next_y = agent(1).sensor.motive.result.rigid(check_i + 1).p(2);
