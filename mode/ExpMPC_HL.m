...
agent = DRONE;
agent.plant = DRONE_EXP_MODEL(agent,Model_Drone_Exp(dt, initial_state, "udp", [1, 252])); %プロポ無線 251 or 252
...
agent.controller.mpc = MPC_CONTROLLER_HL(agent,Controller_MPC_HL(agent)); %最適化手法：QP
...
function result = controller_do(varargin)
    ...
    if varargin{2} == 'a'
        result = controller.mpc.do(varargin); % arming: HLMPC 
    elseif varargin{2} == 't'
        result.hlc = controller.hlc.do(varargin); % takeoff: HLとMPCをどちらも回す
        result.mpc = controller.mpc.do(varargin); % 空で回るだけ．takeoffを実際にするのはHL
        result = result.hlc; % resultに入れる値がhlcだからHLで入力がはいる
    elseif varargin{2} == 'f'
        result = controller.mpc.do(varargin); % flight: HLMPC
    elseif varargin{2} == 'l'
