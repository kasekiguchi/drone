...
arranged_pos = arranged_position([0, 0], N, 1, 0); %arranged_position.mで関数定義
for i = 1:N
    ...
    agent(i).plant = MODEL_CLASS(agent(i),Model_Quat13(dt, initial_state(i), agent(i).id)); %ドローンの状態を更新している MODEL_CLASS.mで定義
%外乱を与える==========
...
%=====================
    agent(i).estimator = EKF(agent(i), Estimator_EKF(agent(i),dt,MODEL_CLASS(agent(i),Model_EulerAngle(dt, initial_state(i), 1)),["p", "q"]));
    agent(i).sensor = MOTIVE(agent(i), Sensor_Motive(1,0, motive));
...
function dfunc(app)
app.logger.plot({1, "p", "pre"},"ax",app.UIAxes,"xrange",[app.time.ts,app.time.te]);
app.logger.plot({2, "p", "pre"},"ax",app.UIAxes2,"xrange",[app.time.ts,app.time.te]);
app.logger.plot({1, "v", "er"},"ax",app.UIAxes3,"xrange",[app.time.ts,app.time.te]);
app.logger.plot({1, "input", ""},"ax",app.UIAxes4,"xrange",[app.time.ts,app.time.t]);
end
