...
for i = 1:N
  ...
  agent(i).sensor.rpos = RANGE_POS_SIM(agent(i),Sensor_RangePos(i,'r',10).param); % 半径r (第二引数) 内の他エージェントの位置を計測 : sim のみ
  agent(i).sensor.rdensity = RANGE_DENSITY_SIM(agent(i),Sensor_RangeD('r',1,'env',env).param); %  半径r (第二引数) 内の重要度を計測 : sim のみ
