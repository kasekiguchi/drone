...
motive = Connector_Natnet('192.168.1.4'); % connect to Motive10号館
% motive = Connector_Natnet('192.168.120.4'); % connect to Motive総研
...
agent.plant = DRONE_EXP_MODEL(agent,Model_Drone_Exp(dt, initial_state, "serial", "3")); %プロポ有線
...
% agent.reference = MY_WAY_POINT_REFERENCE(agent,way_point_ref(readmatrix("waypoint.xlsx",'Sheet','Sheet1_15d3'),5,1));
 agent.reference = TIME_VARYING_REFERENCE(agent,{"My_Case_study_trajectory",{[0,0,1.0]},"HL"});
...
% app.logger.plot({1, "p1-p2", "er"},"ax",app.UIAxes,"xrange",[app.time.ts,app.time.te]);
