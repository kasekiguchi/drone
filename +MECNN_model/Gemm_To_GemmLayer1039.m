classdef Gemm_To_GemmLayer1039 < nnet.layer.Layer & nnet.layer.Formattable
    ...
    methods
        function this = Gemm_To_GemmLayer1039(name, onnxParams)
            this.Name = name;
            this.NumInputs = 2;
            this.OutputNames = {'x35'};
        ...
        function [x35] = predict(this, onnx__Gemm_0, onnx__Gemm_0NumDims)
            ...
            [x35, x35NumDims] = Gemm_To_GemmFcn(onnx__Gemm_0, onnx__Gemm_0NumDims, onnxParams, 'Training', false, ...
                'InputDataPermutation', {['as-is'], ['as-is']}, ...
                'OutputDataPermutation', {['as-is'], ['as-is']});
            if any(cellfun(@(A)~isnumeric(A) && ~islogical(A), {x35}))
                fprintf('Runtime error in network. At least one output of custom layer ''%s'' is a non-numeric, non-logical value.\n', 'Gemm_To_GemmLayer1039');
                error(message('nnet_cnn_onnx:onnx:BadCustomLayerRuntimeOutput', 'Gemm_To_GemmLayer1039'));
            end
            x35 = dlarray(single(x35), repmat('U', 1, max(2, x35NumDims)));
            if ~coder.target('MATLAB')
                x35 = extractdata(x35);
            end
        end
        
        function [x35] = forward(this, onnx__Gemm_0, onnx__Gemm_0NumDims)
            if isdlarray(onnx__Gemm_0)
                onnx__Gemm_0 = stripdims(onnx__Gemm_0);
            end
            onnx__Gemm_0NumDims = numel(onnx__Gemm_0NumDims);
            ...
            onnxParams.Learnables.linears_8_bias = this.linears_8_bias;
            onnxParams.Learnables.linears_8_weight = this.linears_8_weight;
            [x35, x35NumDims] = Gemm_To_GemmFcn(onnx__Gemm_0, onnx__Gemm_0NumDims, onnxParams, 'Training', true, ...
                'InputDataPermutation', {['as-is'], ['as-is']}, ...
                'OutputDataPermutation', {['as-is'], ['as-is']});
            if any(cellfun(@(A)~isnumeric(A) && ~islogical(A), {x35}))
                fprintf('Runtime error in network. At least one output of custom layer ''%s'' is a non-numeric, non-logical value.\n', 'Gemm_To_GemmLayer1039');
...
function [x35, x35NumDims, state] = Gemm_To_GemmFcn(onnx__Gemm_0, onnx__Gemm_0NumDims, params, varargin)
...
%
% [X35] = Gemm_To_GemmFcn(ONNX__GEMM_0, PARAMS)
...
%			network output(s) in X35.
%
% [X35, STATE] = Gemm_To_GemmFcn(ONNX__GEMM_0, PARAMS)
...
[x35, x35NumDims, state] = Gemm_To_GemmGraph1018(onnx__Gemm_0, NumDims.onnx__Gemm_0, Vars, NumDims, Training, params.State);
% Postprocess the output data
[x35] = postprocessOutput(x35, outputDataPerms, anyDlarrayInputs, Training, varargin{:});
end

function [x35, x35NumDims1038, state] = Gemm_To_GemmGraph1018(onnx__Gemm_0, onnx__Gemm_0NumDims1037, Vars, NumDims, Training, state)
% Function implementing the graph 'Gemm_To_GemmGraph1018'
% Update Vars and NumDims from the graph's formal input parameters. Note that state variables are already in Vars.
Vars.onnx__Gemm_0 = onnx__Gemm_0;
NumDims.onnx__Gemm_0 = onnx__Gemm_0NumDims1037;

% Execute the operators:
% Gemm:
[A, B, C, alpha, beta, NumDims.x_linears_0_Gemm_out] = prepareGemmArgs(Vars.onnx__Gemm_0, Vars.linears_0_weight, Vars.linears_0_bias, Vars.Gemmalpha1019, Vars.Gemmbeta1020, 0, 1, NumDims.linears_0_bias);
...
% Gemm:
[A, B, C, alpha, beta, NumDims.x_linears_1_Gemm_out] = prepareGemmArgs(Vars.x_activation_Tanh_ou, Vars.linears_1_weight, Vars.linears_1_bias, Vars.Gemmalpha1021, Vars.Gemmbeta1022, 0, 1, NumDims.linears_1_bias);
...
% Gemm:
[A, B, C, alpha, beta, NumDims.x_linears_2_Gemm_out] = prepareGemmArgs(Vars.x_activation_1_Tanh_, Vars.linears_2_weight, Vars.linears_2_bias, Vars.Gemmalpha1023, Vars.Gemmbeta1024, 0, 1, NumDims.linears_2_bias);
...
% Gemm:
[A, B, C, alpha, beta, NumDims.x_linears_3_Gemm_out] = prepareGemmArgs(Vars.x_activation_2_Tanh_, Vars.linears_3_weight, Vars.linears_3_bias, Vars.Gemmalpha1025, Vars.Gemmbeta1026, 0, 1, NumDims.linears_3_bias);
...
% Gemm:
[A, B, C, alpha, beta, NumDims.x_linears_4_Gemm_out] = prepareGemmArgs(Vars.x_activation_3_Tanh_, Vars.linears_4_weight, Vars.linears_4_bias, Vars.Gemmalpha1027, Vars.Gemmbeta1028, 0, 1, NumDims.linears_4_bias);
...
% Gemm:
[A, B, C, alpha, beta, NumDims.x_linears_5_Gemm_out] = prepareGemmArgs(Vars.x_activation_4_Tanh_, Vars.linears_5_weight, Vars.linears_5_bias, Vars.Gemmalpha1029, Vars.Gemmbeta1030, 0, 1, NumDims.linears_5_bias);
...
% Gemm:
[A, B, C, alpha, beta, NumDims.x_linears_6_Gemm_out] = prepareGemmArgs(Vars.x_activation_5_Tanh_, Vars.linears_6_weight, Vars.linears_6_bias, Vars.Gemmalpha1031, Vars.Gemmbeta1032, 0, 1, NumDims.linears_6_bias);
...
% Gemm:
[A, B, C, alpha, beta, NumDims.x_linears_7_Gemm_out] = prepareGemmArgs(Vars.x_activation_6_Tanh_, Vars.linears_7_weight, Vars.linears_7_bias, Vars.Gemmalpha1033, Vars.Gemmbeta1034, 0, 1, NumDims.linears_7_bias);
...
% Gemm:
[A, B, C, alpha, beta, NumDims.x35] = prepareGemmArgs(Vars.x_activation_7_Tanh_, Vars.linears_8_weight, Vars.linears_8_bias, Vars.Gemmalpha1035, Vars.Gemmbeta1036, 0, 1, NumDims.linears_8_bias);
Vars.x35 = alpha*B*A + beta*C;

% Set graph output arguments from Vars and NumDims:
x35 = Vars.x35;
x35NumDims1038 = NumDims.x35;
...
function [x35] = postprocessOutput(x35, outputDataPerms, anyDlarrayInputs, Training, varargin)
% Set output type:
...
x35 = permuteOutputVar(x35, outputDataPerms{1}, 2);
end
