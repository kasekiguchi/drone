                     ...
                     idx_global_start, int32_T idx_global_dest)
{
  ...
  b_st.prev = &st;
  b_st.tls = st.tls;
  if ((idx_global_start < 1) || (idx_global_start > 617)) {
    emlrtDynamicBoundsCheckR2012b(idx_global_start, 1, 617, &rc_emlrtBCI, sp);
  }

  if ((idx_global_dest < 1) || (idx_global_dest > 617)) {
    emlrtDynamicBoundsCheckR2012b(idx_global_dest, 1, 617, &sc_emlrtBCI, sp);
  }
  ...
  for (idx = 0; idx < b; idx++) {
    obj->ATwset[idx + 485 * (idx_global_dest - 1)] = obj->ATwset[idx + 485 *
      (idx_global_start - 1)];
  }
