...
static emlrtRSInfo ak_emlrtRSI = { 1,  /* lineNo */
...
void checkStoppingAndUpdateFval(F_HL_MPCfuncStackData *SD, const emlrtStack *sp,
  int32_T *activeSetChangeID, const real_T f[485], e_struct_T *solution,
  ...
  runTimeOptions_MaxIterations, boolean_T updateFval)
{
  ...
  if (solution->iterations - solution->iterations / 50 * 50 == 0) {
    ...
    if (solution->maxConstr > 1.0E-6) {
      st.site = &ak_emlrtRSI;
      g_xcopy(&st, objective->nvar, solution->xstar, solution->searchDir);
      st.site = &ak_emlrtRSI;
