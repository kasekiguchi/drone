...
void removeAllIneqConstr(const emlrtStack *sp, g_struct_T *obj)
{
  ...
  for (idx_global = idxStartIneq; idx_global <= idxEndIneq; idx_global++) {
    if ((idx_global < 1) || (idx_global > 617)) {
      emlrtDynamicBoundsCheckR2012b(idx_global, 1, 617, &fd_emlrtBCI, sp);
    }

    i = obj->Wid[idx_global - 1];
    if ((i < 1) || (i > 6)) {
      emlrtDynamicBoundsCheckR2012b(obj->Wid[idx_global - 1], 1, 6, &gd_emlrtBCI,
        sp);
    }

    i = (obj->isActiveIdx[i - 1] + obj->Wlocalidx[idx_global - 1]) - 1;
    if ((i < 1) || (i > 617)) {
      emlrtDynamicBoundsCheckR2012b(i, 1, 617, &hd_emlrtBCI, sp);
    }
