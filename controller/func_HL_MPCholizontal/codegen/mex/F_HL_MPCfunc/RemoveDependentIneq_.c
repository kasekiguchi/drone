  ...
  k_struct_T *qrmanager, b_struct_T *memspace, real_T tolfactor)
{
  ...
  if ((workingset->nWConstr[2] + workingset->nWConstr[3]) + workingset->
    ...
    for (idx = 0; idx < nFixedConstr; idx++) {
      i = idx + 1;
      if ((i < 1) || (i > 617)) {
        emlrtDynamicBoundsCheckR2012b(i, 1, 617, &tc_emlrtBCI, sp);
      }
    ...
    for (idx = a; idx <= nActiveConstr; idx++) {
      if ((idx < 1) || (idx > 617)) {
        emlrtDynamicBoundsCheckR2012b(idx, 1, 617, &tc_emlrtBCI, sp);
      }
    ...
    for (idx = workingset->nActiveConstr; idx > workingset->nVar; idx--) {
      nDepIneq++;
      if ((idx < 1) || (idx > 617)) {
        emlrtDynamicBoundsCheckR2012b(idx, 1, 617, &uc_emlrtBCI, sp);
      }

      if (nDepIneq > 617) {
        emlrtDynamicBoundsCheckR2012b(618, 1, 617, &tc_emlrtBCI, sp);
      }
    ...
    if (idx <= workingset->nVar) {
      ...
      while ((!exitg1) && (idx > nFixedConstr)) {
        ...
        if (muDoubleScalarAbs(qrmanager->QR[(idx + 617 * (idx - 1)) - 1]) < tol)
        {
          nDepIneq++;
          if (nDepIneq > 617) {
            emlrtDynamicBoundsCheckR2012b(618, 1, 617, &tc_emlrtBCI, sp);
    ...
    for (idx = nDepIneq; idx >= 1; idx--) {
      ...
      if ((i < 1) || (i > 617)) {
        emlrtDynamicBoundsCheckR2012b(memspace->workspace_int[idx - 1], 1, 617,
          &ic_emlrtBCI, &st);
      }
      ...
      i = workingset->Wid[a];
      if ((i < 1) || (i > 6)) {
        emlrtDynamicBoundsCheckR2012b(i, 1, 6, &cc_emlrtBCI, &st);
      }
      ...
      if ((nActiveConstr < 1) || (nActiveConstr > 5)) {
        emlrtDynamicBoundsCheckR2012b(nActiveConstr, 1, 5, &mc_emlrtBCI, &st);
      }
