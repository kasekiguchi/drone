...
real_T computePrimalFeasError(const emlrtStack *sp, const real_T cIneq[176],
  const real_T cEq[88], const int32_T finiteLB[485], int32_T mLB)
{
  real_T feasError;
  int32_T idx;
  emlrtStack st;
  ...
  for (idx = 0; idx < mLB; idx++) {
    if ((finiteLB[idx] < 1) || (finiteLB[idx] > 0)) {
      emlrtDynamicBoundsCheckR2012b(finiteLB[idx], 1, 0, &eb_emlrtBCI, sp);
    }

    if ((finiteLB[idx] < 1) || (finiteLB[idx] > 132)) {
      emlrtDynamicBoundsCheckR2012b(finiteLB[idx], 1, 132, &db_emlrtBCI, sp);
    }
  }
