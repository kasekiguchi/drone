...
boolean_T isDeltaXTooSmall(const emlrtStack *sp, const real_T xCurrent[110],
  const real_T delta_x[307], int32_T nVar)
{
  boolean_T tf;
  int32_T idx;
  boolean_T exitg1;
  ...
  while ((!exitg1) && (idx <= nVar - 1)) {
    i = idx + 1;
    if ((i < 1) || (i > 110)) {
      emlrtDynamicBoundsCheckR2012b(i, 1, 110, &j_emlrtBCI, sp);
    }
