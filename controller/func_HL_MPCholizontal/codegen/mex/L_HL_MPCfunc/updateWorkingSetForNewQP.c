  ...
  const real_T cIneq[20], const real_T cEq[88])
{
  ...
  for (idx = 0; idx < 88; idx++) {
    for (b_i = 0; b_i <= i; b_i++) {
      i1 = iEq0 + b_i;
      if ((i1 < 1) || (i1 > 27016)) {
        emlrtDynamicBoundsCheckR2012b(i1, 1, 27016, &d_emlrtBCI, sp);
      }

      i2 = iw0 + b_i;
      if ((i2 < 1) || (i2 > 93635)) {
        emlrtDynamicBoundsCheckR2012b(i2, 1, 93635, &c_emlrtBCI, sp);
      }
  ...
  if (WorkingSet->nActiveConstr > 88) {
    ...
    for (idx = 89; idx <= iw0; idx++) {
      if (idx > 305) {
        emlrtDynamicBoundsCheckR2012b(idx, 1, 305, &v_emlrtBCI, sp);
      }
      ...
      switch (WorkingSet->Wid[idx - 1]) {
       case 4:
        ...
        if ((i < 1) || (i > 307)) {
          emlrtDynamicBoundsCheckR2012b(WorkingSet->Wlocalidx[idx - 1], 1, 307,
            &x_emlrtBCI, sp);
        }
       ...
       default:
        ...
        if (WorkingSet->Wlocalidx[idx - 1] >= 20) {
          st.site = &h_emlrtRSI;
          xcopy(&st, nVar, WorkingSet->Aineq, 5834, WorkingSet->ATwset, 307 *
                (idx - 1) + 1);
        }
        break;
      }
