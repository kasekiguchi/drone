...
void compute_lambda(const emlrtStack *sp, real_T workspace[94249], e_struct_T
  ...
  if (qrmanager->ncols > 0) {
    ...
    if (nonDegenerate) {
      ...
      if (qrmanager->mrows < qrmanager->ncols) {
        ...
        while ((!exitg3) && (idx > qrmanager->mrows)) {
          if ((qrmanager->mrows < 1) || (qrmanager->mrows > 307)) {
            emlrtDynamicBoundsCheckR2012b(qrmanager->mrows, 1, 307, &id_emlrtBCI,
              &st);
          }

          if ((idx < 1) || (idx > 307)) {
            emlrtDynamicBoundsCheckR2012b(idx, 1, 307, &id_emlrtBCI, &st);
          }

          if (muDoubleScalarAbs(qrmanager->QR[(qrmanager->mrows + 307 * (idx - 1))
                                - 1]) >= tol) {
            idx--;
          } else {
            exitg3 = true;
      ...
      if (guard1) {
        do {
          ...
          if (idx >= 1) {
            ...
            if (muDoubleScalarAbs(qrmanager->QR[(idx + 307 * (idx - 1)) - 1]) >=
                tol) {
              idx--;
            } else {
              nonDegenerate = (idx == 0);
    ...
    if (!nonDegenerate) {
      ...
      for (idx = 0; idx < nActiveConstr; idx++) {
        b_idx = idx + 1;
        if ((b_idx < 1) || (b_idx > 94249)) {
          emlrtDynamicBoundsCheckR2012b(b_idx, 1, 94249, &jd_emlrtBCI, sp);
        }

        if (b_idx > 305) {
          emlrtDynamicBoundsCheckR2012b(b_idx, 1, 305, &kd_emlrtBCI, sp);
        }
