...
f = FL_extend(x,physicalParam);
g = GL_extend(x,physicalParam);
%g= [g1 g2 g3 g4];
%% 1st layer
clc
...
dddh2 = simplify(LieD(ddh2,f1,x)+diff(ddh2,t));
dddh3 = simplify(LieD(ddh3,f1,x)+diff(ddh3,t));
d4h2 = LieD(dddh2,f1,x)+diff(dddh2,t);
d4h3 = LieD(dddh3,f1,x)+diff(dddh3,t);
...
%% Set variables for output functions
    syms Xd1 Xd2 Xd3 Xd4 dXd1 dXd2 dXd3 dXd4 ddXd1 ddXd2 ddXd3 ddXd4 dddXd1 dddXd2 dddXd3 dddXd4 ddddXd1 ddddXd2 ddddXd3 ddddXd4 dddddXd1 dddddXd2 dddddXd3 dddddXd4 ddddddXd1 ddddddXd2 ddddddXd3 ddddddXd4 real
    syms V1 V2 V3 V4 dV1 ddV1 dddV1 ddddV1 dddddV1 real
    XD = {Xd1 Xd2 Xd3 Xd4 dXd1 dXd2 dXd3 dXd4 ddXd1 ddXd2 ddXd3 ddXd4 dddXd1 dddXd2 dddXd3 dddXd4 ddddXd1 ddddXd2 ddddXd3 ddddXd4 dddddXd1 dddddXd2 dddddXd3 dddddXd4 ddddddXd1 ddddddXd2 ddddddXd3 ddddddXd4};
...
clc
    ...
    matlabFunction(subs([-F1*[h1;dh1],-F1*A1*[h1;dh1],-F1*A1*A1*[h1;dh1],-F1*A1*A1*A1*[h1;dh1],-F1*A1*A1*A1*A1*[h1;dh1]], [xdRef], [XD]),'file','Vf_SuspendedLoad_extend.m','vars',{x cell2sym(XD) physicalParam F1},'outputs',{'V1'});
    matlabFunction(subs([-F2*[h2;dh2;ddh2;dddh2;d4h2;d5h2],-F3*[h3;dh3;ddh3;dddh3;d4h3;d5h3],-F4*[h4;dh4]], [xdRef vInput1], [XD V1v]),'file','Vs_SuspendedLoad_extend.m','vars',{x cell2sym(XD) cell2sym(V1v) physicalParam F2 F3 F4},'outputs',{'cV2'});
    A1 = expm([0,1;0,0]*dt)-int(expm([0,1;0,0]*(dt-k))*[0;1],k,[0,dt])*F1; % closed loop discrete
    matlabFunction(subs([-F1*[h1;dh1],-F1*A1*[h1;dh1],-F1*A1*A1*[h1;dh1],-F1*A1*A1*A1*[h1;dh1],-F1*A1*A1*A1*A1*[h1;dh1]], [xdRef], [XD]),'file','Vfd_SuspendedLoad_extend.m','vars',{dt x cell2sym(XD) physicalParam F1},'outputs',{'V1'});
...
% % Usage: u = Uf(...) + Us(...)
%     matlabFunction(subs(H(:,1)*(-alpha1+v1(t)), [xdRef vInput1], [XD V1v]),'file','Uf_SuspendedLoad_extend.m','vars',{x cell2sym(XD) cell2sym(V1v) physicalParam},'outputs',{'U1'});
%     matlabFunction(subs(H(:,2:4), [xdRef vInput1 v2(t) v3(t) v4(t)], [XD V1v [V2 V3 V4]]),'file','H234_SuspendedLoad_extend.m','vars',{x cell2sym(XD) cell2sym(V1v) [V2;V3;V4] physicalParam},'outputs',{'U2'});
    matlabFunction(subs(inv(beta2), [xdRef vInput1 v2(t) v3(t) v4(t)], [XD V1v [V2 V3 V4]]),'file','inv_beta2_SuspendedLoad_extend.m','vars',{x cell2sym(XD) cell2sym(V1v) [V2;V3;V4] physicalParam},'outputs',{'U2'});
    matlabFunction(subs((-alpha2+[v2(t);v3(t);v4(t)]), [xdRef vInput1 v2(t) v3(t) v4(t)], [XD V1v [V2 V3 V4]]),'file','v_SuspendedLoad_extend.m','vars',{x cell2sym(XD) cell2sym(V1v) [V2;V3;V4] physicalParam},'outputs',{'U2'});
