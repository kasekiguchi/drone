classdef MCMPC_controller_original <CONTROLLER_CLASS
    ...
    methods
        function obj = MCMPC_controller_original(self, param)
            %---- 変数定義 ----%
            obj.self = self; 
            %--MPCパラメータ設定--%
            ...
            obj.state.p_data = zeros(3,obj.param.H,obj.param.particle_num);
            obj.state.p_data = repmat(reshape(obj.state.p_data,[1,size(obj.state.p_data)]),3,1);%(先輩に聞く)
        ...
        function result = do(obj,param)
            ...
            %-- 現在状態の取得
            obj.previous_state.x0 = obj.self.estimator.result.state.get();%(先輩に聞く)
            %(上の式の右辺はagent(どこからでもアクセスできる)から引っ張ってきている)
            %obj.self:agentにアクセス，estimator → result →
%             state:agentの中のestimatorの中のresultの中のstateにアクセス，get()で中身をすべて
%             obj.previous_state.x0に代入
...
%             obj.result = obj.input.u1;
        ...
        %-- 評価関数
        function [MCeval] = objective(obj, i)   %()
            X = obj.state.state_data;       %12 * 10 = 12状態×10ホライズン
            U = obj.input.u(:,:,m);         %4  * 10 = 4入力(プロペラの数)×10ホライズン
