function plot_KLResult(file,flg)
%% plot_KLRESULT
% main_KoopmanLinearization.mで作成したシミュレーション結果を出力
% file : シミュレーション結果 Cell
%   file{n}         出力したいsimResult
%   file{1}のReferenceを全体のReferenceとして扱う
% flg : 各フラグ 空なら0として扱う
%   .calcFile1RMSE  file{1}に読み込んだデータのRMSEを求めるか
%   .ylimHold       指定した値にylimを固定するか
%   .xlimHold       指定した値にxlimを固定するか
%   .figureSave     figure を外部出力するか
%
% 後々各ルールを別ファイルで生成，管理してこの関数ではPLOTだけする予定

%% flag
if isfield(flg,'calcFile1RMSE')==0
    flg.calcFile1RMSE = 0;
end
if isfield(flg,'ylimHold')==0
    flg.ylimHold = 0;
end
if isfield(flg,'xlimHold')==0
    flg.xlimHold = 1;
end
if isfield(flg,'figureSave')==0
    flg.figureSave = 0;
end
...
for i = 1:HowmanyFile
    ...
    elseif indexcheck ~= file{i}.simResult.initTindex
            ...
            [key] = input('[q] を入力して強制終了，[その他]で無視して続行します');
            switch key
                case 'q'
                    quit
                otherwise
