function [U,dqid,ddqid] = CooperativeSuspendedLoadController_3(x,qi,R0,Ri,R0d,xd,K,P,Pdagger,tdqid,tddqid)
 % x : state
 % qi : subpart of x 3xN
...
b1 = repmat(xd(4:6),1,3);% + 0.1*cell2mat(arrayfun(@(i) Ri(:,:,i)*[1;0;0],1:3,'UniformOutput',false)); % Caution : problem if dx0d = - 10*xi.
b1(:,vecnorm(b1,2,1)==0) = repmat([1;0;0],1,sum(vecnorm(b1,2,1)==0));
