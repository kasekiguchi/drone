classdef MPC_CONTROLLER_KOOPMAN_quadprog_simulation_v2 < handle
...
%         options
    ...
    methods
        function obj = MPC_CONTROLLER_KOOPMAN_quadprog_simulation_v2(self, param)
            %-- 変数定義
        ...
        function result = do(obj,varargin)
            ...
            %% ------------------------------------------------------------
            % 最適化部分の関数化とmex化 
            % Param = struct('current_state',obj.current_state,'ref',obj.reference.xr,'qpH', obj.qpparam.H,...
            %     'qpF', obj.qpparam.F,'lb',obj.param.input.lb,'ub',obj.param.input.ub,...
            %     'previous_input',obj.previous_input,'H',obj.H,'F',obj.param.F);
            ...
            Param = struct('current_state',obj.current_state,'A',obj.param.A,'B',obj.param.B,...
                'C',obj.param.C,'weight',obj.weight,'weightF',obj.weightF,'weightR',obj.weightR,...
                'ref',obj.reference.xr,'H',obj.H,'F',obj.param.F);
            [H, f] = change_equation(Param);
            A = [];
            b = [];
            Aeq = [];
            beq = [];
            lb = repmat(obj.input.lb, obj.H, 1); % 下限制約
            ub = repmat(obj.input.ub, obj.H, 1); % 上限制約
