classdef MPC_KOOPMAN_CVXGEN < handle
...
%         options
    ...
    methods
        function obj = MPC_KOOPMAN_CVXGEN(self, param)
            %-- 変数定義
        ...
        function result = do(obj,varargin)
            ...
            %% cvxgen
            params.A = obj.A(1:12, 1:12);
            params.B1 = obj.B(1:12, 1);
            params.B2 = obj.B(1:12, 2:4);
            params.C = obj.C(1:12, 1:12);
            params.Q = obj.weight; % 12*12に拡張
            params.Q_f = obj.weightF;
            params.R1 = obj.weightR(1,1);
            params.R2 = obj.weightR(2:4,2:4);
            params.x_0 = obj.current_state; % 1*12
            params.ref = obj.reference.xr(1:12,1);
            settings.verbose = 0;
            [vars, status] = csolve(params, settings);
            var_get = reshape(cell2mat(vars.x), 12, []);
            var = var_get(:, 1:obj.H);
            obj.previous_input = var;
            obj.result.input = [vars.u_th{1};vars.u_tr{1}];
            fval = status.optval;
            exitflag = 10;
