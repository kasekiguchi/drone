classdef HLController_quadcopter_Suspended_Load_Extend < CONTROLLER_CLASS
    ...
    methods
        function obj = HLController_quadcopter_Suspended_Load_Extend(self,param)
            obj.self = self;
            obj.param = param;
...
%             tmp(4)];
            ...
            if isfield(Param,'dt')
                dt = Param.dt;
                vf = Vfd_SuspendedLoad_extend(dt,x,xd',P,F1);
            else
                vf = Vf_SupendedLoad_extend(x,xd',P,F1);
            end
            vs = Vs_SuspendedLoad_extend(x,xd',vf,P,F2,F3,F4);
            uf = Uf_SuspendedLoad_extend(x,xd',vf,P);
            
            h234 = H234_SuspendedLoad_extend(x,xd',vf,vs',P);
            invbeta2 = inv_beta2_SuspendedLoad_extend(x,xd',vf,vs',P);
            a = v_SuspendedLoad_extend(x,xd',vf,vs',P);
            us = h234*invbeta2*a;
