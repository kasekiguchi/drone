...
for i = 1:N
    ...
    if fExp
        agent(i) = DRONE(Model_Drone_Exp(dt,initial_state(i),"udp",[100,252]),DRONE_PARAM("DIATONE")); % for exp % 機体番号（ESPrのIP）%"udp",[25]
...
%     agent(i).set_property("reference", Reference_Point_FH_COST());%東急のコスト検証用
