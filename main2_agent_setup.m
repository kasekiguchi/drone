...
for i = 1:N
    ...
    %% model
    % set control model
    agent(i).set_model(Model_EulerAngle(dt,initial_state(i), i),DRONE_PARAM("DIATONE")); % オイラー角モデル
...
%     agent(i).set_property("sensor",Sensor_RangeD('r',3)); %  半径r (第二引数) 内の重要度を計測 : sim のみ
    agent(i).set_property("sensor",Sensor_Map_3D(bird,'d',10)); % 測定距離d（第二引数宇）内の重要度を計測 : sim のみ 
