...
%         agent(i) = DRONE(Model_Quat13(dt,initial(i),i),DRONE_PARAM("DIATONE")); % unit quaternionのプラントモデル : for sim
        ...
        agent(i) = DRONE(Model_Suspended_Load(dt,initial(i),i,DRONE_PARAM("DIATONE")),DRONE_PARAM("DIATONE","parameter_name",["mass","Length","jx","jy","jz","gravity","km1","km2","km3","km4","k1","k2","k3","k4","loadmass","cableL","ex","ey","ez"])); % 牽引物込みのプラントモデル : for sim
...
%     agent(i).set_estimator("estimator", Estimator_EKF(agent(i), ["p", "q"], [1e-5, 1e-8])); % （剛体ベース）EKF
    ...
    agent(i).set_property("estimator",Estimator_EKF(agent(i),["p","q","pL","pT"],[1e-8,1e-8,1e-8,1e-7])); % （剛体ベース）EKF
...
%     agent(i).set_property("reference",Reference_Time_Varying("Case_study_trajectory",{[0;0;1],2})); % ハート形[x;y;z]永久
    agent(i).set_property("reference",Reference_Time_Varying_Suspended_Load("Case_study_trajectory",{[0;0;1],1})); % ハート形[x;y;z]永久
    ...
    HL_LoadControlSetting = Controller_HL_Suspended_Load(dt);
    HL_LoadParam = HL_LoadControlSetting.param;
