...
motive = NATNET_CONNECTOR_SIM(1, dt, 0); % imitation of Motive camera (motion capture system)
...
if ~modeType
    ...
    Ucost = cell2mat(arrayfun(@(N) logger.Data.agent.controller.result{N}.bestcost(2), 1:round(time.t/dt),'UniformOutput',false));
    N = length(cost);
    ...
    % gui.logger = logger; P = Controller_MPC_KMC(dt, model_file, agent); mojamoja(gui, P, 'xz');
end
%%
function dfunc(app)
