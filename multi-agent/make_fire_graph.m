function [E,ES,EF,E2] = make_fire_graph(nx,ny,meas,W,wind1,wind2,mapd,maxv)
...
winda = wind1 + mapd;  %wind angle 22.5°刻み
Til = (pi()/16) * winda;  %角度の設定 Tilt
if winda >=32
    winda = rem(winda,32);
end
if winda>=17 && 31>=winda    %東西判定
    windEW = -abs(windv);
elseif winda>=1 && 15>=winda
    windEW = abs(windv);
end
if rem(winda,16) == 0
    windEW = 0;
end
if winda>=25 || 7>=winda    %南北判定
    windSN = abs(windv);
elseif winda>=9 && 23>=winda
    windSN = -abs(windv);
end
if winda == 8 || winda == 24
    windSN = 0;
