...
wind_time = kron((1:size(wind,1))',ones(hours(table2array(winddata(end,1)-winddata(end-1,1)))*obj.step_per_h,1));
  ...
  wind_dir = file_name(1);
  i = find(wdir_base == wind_dir);
  wind(1,1) = (pi/180)*rem(i,32)*360/32 - obj.shape_opts.north_dir; % wind direction : 0 = 'south', 90 = 'west'
  wind(1,2) = double(file_name(2));
  wind_time = [];
end
end
