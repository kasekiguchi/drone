for i = 1:N
    %% generate Drone instance(ドローンのプラントモデルや物理パラメータを設定)
...
%     agent(i).set_model(Model_Quat13(dt,initial(i),i)); % オイラーパラメータ（unit quaternion）モデル
    ...
    %% set input_transform property(実機の場合，計算した推力からプロポのスロットルに変換)
    ...
    %% set sensors property(motiveから情報を持ってくる)
    agent(i).sensor = [];
    ...
    %% set estimator property(センサーで取れない状態を推定，推定方法の選定)
    agent(i).estimator = [];
...
%     agent(i).set_property("reference",Reference_Time_Varying("Case_study_trajectory",[1;0;1])); % ハート形[x;y;z]永久
    agent(i).set_property("reference",Reference_Time_Varying("Komatsu_study_trajectory",[1;0;1;10])); % 円旋回 コマツ
%     agent(i).set_property("reference",Reference_Time_Varying_Suspended_Load("Case_study_trajectory",[1;0;1])); % ハート形[x;y;z]永久
    ...
    %% set controller property(制御器の設定)
    agent(i).controller = [];
    %-- sample 通さずに実行
