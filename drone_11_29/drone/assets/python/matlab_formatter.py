...
class Formatter:
    ...
    # indentation
    ilvl = 0
    cilvl = [] # comment indentation level
    bcline = [] # block structure changed line number
    istep = []
    fstep = []
    iwidth = 0
    matrix = 0
    ...
    # take care of indentation and call format(line)
    def formatLineComment(self, line, cilvl):
        m = re.match(self.alllinecomment, line)
        if (m and not re.match(self.linecomment, line)):
            return re.sub('%\s*',(' '*(cilvl-m.end()+1))+'% ',line,1)
        return line

    # format file from line 'start' to line 'end'
    def formatFile(self, filename, start, end, aligncomments):
        ...
        if filename == '-':
            ...
            with open(filename, 'rb') as f:
                m = chardet.detect(f.read())
                #print(m['encoding'])
            with open(filename, 'r', encoding=m['encoding']) as f:
                fulllines = f.readlines()    
            rlines = fulllines[start-1:end]
        ...
        if m:
            ...
            rlines[0] = m.group(2)
            if not aligncomments:
                self.bcline += [1]
                self.cilvl += [m.start()]

        blank = True
        li = 0 # line number 
        bcnum = -1 # block structure changed number
        for line in fulllines:
            li += 1
            if not (li >= start and li <= start+len(rlines)-1):
                if aligncomments:
                ...
                if (offset != 0): # if block structure is changed
                    bcnum += 1
                    if (not aligncomments):
                        self.bcline += [li] # add block structure changed line number
                        # get initial comment indent lvl
                        p = r'^\s*\S'
                        m = re.match(p, line)
                        if m:
                            #self.cilvl = [len(m.group(1))//self.iwidth]
                            self.cilvl += [m.end()]

                if aligncomments:
                    line = self.formatLineComment(line,self.cilvl[bcnum])
                elif (re.search('%',line)):
                    m = re.search('%',line)
                    self.cilvl[bcnum] = max(m.start(),self.cilvl[bcnum])
    ...
    if len(sys.argv) < 2:
        ...
        for arg in sys.argv[2:]:
            ...
            if any(char.isdigit() for char in value):
                ...
                value = False
            roptions[key.strip()] = value
        for key in options:
            if not ('--%s' % (key) in roptions):
                roptions['--%s' % (key)] = options[key]
        indent = roptions['--indentWidth']
        start = roptions['--startLine']
        end = roptions['--endLine']
        sep = roptions['--separateBlocks']
        mode = indentModes.get(roptions['--indentMode'],
                               indentModes['all_functions'])
        formatter = Formatter(indent, sep, mode)
        formatter.formatFile(sys.argv[1], start, end, 0)
        #print(formatter.cilvl)
        #print(formatter.bcline)
        formatter.formatFile(sys.argv[1], start, end, 1)
