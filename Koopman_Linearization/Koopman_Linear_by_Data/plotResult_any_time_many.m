...
for i = 1:HowmanyFile
    ...
    tlength = file{i}.simResult.initTindex + startIdx:file{i}.simResult.initTindex+stepN-1 + startIdx;
    %検証用データの初期値を観測量に通して次元を合わせてる
    if i == 1; simResult.Z(:,startIdx) = F(file{WhichRef}.simResult.reference.X(:,startIdx)); 
    elseif i == 2; simResult.Z(:,startIdx) = F2(file{WhichRef}.simResult.reference.X(:,startIdx)); 
    end
    simResult.Xhat(:,startIdx) = file{WhichRef}.simResult.reference.X(:,startIdx);
    for j = startIdx:startIdx+stepN
        simResult.Z(:,j+1) = file{i}.est.A * simResult.Z(:,j) + file{i}.est.B * file{WhichRef}.simResult.U(:,j); 
    end
    simResult.Xhat = file{i}.est.C * simResult.Z;
    % 読み込んだ情報(file{i}.simResult.state)の書き換え 
    if size(file{WhichRef}.Data.X,1)==13
        file{i}.simResult.state.p = simResult.Xhat(1:3,:);
        file{i}.simResult.state.q = simResult.Xhat(4:7,:);
        file{i}.simResult.state.v = simResult.Xhat(8:10,:);
        file{i}.simResult.state.w = simResult.Xhat(11:13,:);
    else
        file{i}.simResult.state.p = simResult.Xhat(1:3,:);
        file{i}.simResult.state.q = simResult.Xhat(4:6,:);
        file{i}.simResult.state.v = simResult.Xhat(7:9,:);
        file{i}.simResult.state.w = simResult.Xhat(10:12,:);
    end
end
...
% 入力されたファイル数分ループ
for i = 1:HowmanyFile
    if i == 1; plot(timeRange, file{i}.simResult.state.p(:,tlength),file{i}.markerSty,'MarkerSize',7,'LineWidth',1,'LineStyle','--');
    elseif i == 2; plot(timeRange, file{i}.simResult.state.p(:,tlength),file{i}.markerSty,'MarkerSize',7,'LineWidth',1,'LineStyle',':');
    end
...
grid on
for i = 1:HowmanyFile
    if i == 1; plot(timeRange, file{i}.simResult.state.q(:,tlength),file{i}.markerSty,'MarkerSize',7,'LineWidth',1,'LineStyle','--');
    elseif i == 2; plot(timeRange, file{i}.simResult.state.q(:,tlength),file{i}.markerSty,'MarkerSize',7,'LineWidth',1,'LineStyle',':');
    end
...
grid on
for i = 1:HowmanyFile
    if i == 1; plot(timeRange, file{i}.simResult.state.v(:,tlength),file{i}.markerSty,'MarkerSize',7,'LineWidth',1,'LineStyle','--');
    elseif i == 2; plot(timeRange, file{i}.simResult.state.v(:,tlength),file{i}.markerSty,'MarkerSize',7,'LineWidth',1,'LineStyle',':');
    end
...
grid on
for i = 1:HowmanyFile
    if i == 1; plot(timeRange, file{i}.simResult.state.w(:,tlength),file{i}.markerSty,'MarkerSize',7,'LineWidth',1,'LineStyle','--');
    elseif i == 2; plot(timeRange, file{i}.simResult.state.w(:,tlength),file{i}.markerSty,'MarkerSize',7,'LineWidth',1,'LineStyle',':');
    end
