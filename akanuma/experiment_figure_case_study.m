...
for i = 3:12
filename = strcat('exp', num2str(i), '_hovering'); %
...
close all
clear Ref
flg.figtype = 1; % 0:subplot/1:figure
flg.timerange = 1; % 0:実際の時間/1:0から
flg.plotmode = 1; % 0:デフォルトの描画/1:innner_input, xy, xyzを追加
...
if flg.plotmode == 0; m=2; n=3; else m=4; n=2; end
...
plot(logt, Est(10:12,:)); hold on; plot(logt, zeros(3, length(logt)), '--'); hold off;
background_color(phase, -0.1, gca, logt, log.Data.phase); 
xlabel("Time [s]"); ylabel("Angular velocity [rad/s]"); legend("roll", "pitch", "yaw", "roll.reference", "pitch.reference", "yaw.reference", "Location","best");
grid on; xlim([logt(1), logt(end)]); ylim([-inf inf]);
...
if flg.figtype; figure(7); else subplot(m,n,7); end
% calculation time
...
%%
type = {'p', 'q', 'v', 'w', 'thrust', 'torque', 'calculation', 'xy'};
for i = 1:8
    f(i) = figure(i);
    % mkdir(strcat(filename, '/jpg/')); 
    mkdir(strcat(filename, '/png/'));
    mkdir(strcat(filename, '/fig/'));
    % saveas(i, strcat(filename, '/jpg/',filename, '_', type{i},'.jpg')); 
    saveas(i, strcat(filename, '/png/',filename, '_', type{i},'.png')); 
    savefig(f(i), strcat(filename, '/fig/',filename, '_', type{i}));
end 
fprintf('saved\n');
end
