classdef FOR_LOAD < SENSOR_CLASS
      properties
        ...
        tt0
        tl0
        tte = 10;%ÉZÉìÉTÅ[ílÇâΩïbÇ≈100%égÇ§Ç©
        tle = 5;%ÉZÉìÉTÅ[ílÇâΩïbÇ≈0%égÇ§Ç©
        ratet
        ratel
        ilength %é¿ç€ÇÃxyà íuÇégÇ¢énÇﬂÇÈÇ©
        inispL
    end
    ...
    methods
        function obj = FOR_LOAD(self,varargin)
            ...
            obj.ratet = 1/obj.tte^2;%ìÒéüä÷êîÇ≈0-1ÇÃä‘Ç≈ïœâªÇ∑ÇÈ
            obj.ratel = 1/obj.tle^2;%ìÒéüä÷êîÇ≈0-1ÇÃä‘Ç≈ïœâªÇ∑ÇÈ
        end
        
        function [result]=do(obj,varargin)
            %   param : optional
            if isempty(obj.ilength)
                obj.self.sensor.motive.result.rigid(1).p(3) + 0.1;%å°à¯ï®ÇÃçÇÇ≥+íËêî
            end
            ...
            elseif strcmp(varargin{1}{2},'l')&& sp(3) - sp(3) < 0.7*obj.self.parameter.get("cableL")%landing
