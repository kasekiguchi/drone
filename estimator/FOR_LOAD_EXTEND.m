classdef FOR_LOAD_EXTEND < ESTIMATOR_CLASS
      properties
        state
        result
        rigid_num
    ...
    methods
        function obj = FOR_LOAD_EXTEND(self,varargin)
            obj.self= self;
            if ~isempty(varargin)
        ...
        function [result]=do(obj,~)
            ...
            obj.result.state.pT = (obj.result.state.pL-(obj.result.state.p-RodriguesQuaternion(obj.result.state.q)*[0;0;obj.self.model.param(end)]))/norm(obj.result.state.pL-(obj.result.state.p-RodriguesQuaternion(obj.result.state.q)*[0;0;obj.self.model.param(end)]));
            result = obj.result;
        end
        function show()
        end
