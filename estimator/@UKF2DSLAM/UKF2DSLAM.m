classdef UKF2DSLAM < ESTIMATOR_CLASS
    ...
    methods
        function obj = UKF2DSLAM(self,param)
            ...
            obj.constant.LineThreshold = 0.3;%0.3;%0.3; %"ax + by + c"の誤差を許容する閾値
        ...
        function [result]=do(obj,~,~)
            ...
            %% sigma point update
            sol = arrayfun(@(i) ode45(@(t,x) model.method(x,u,obj.self.parameter), [0 obj.dt], Kai(1:obj.n,i)), 1:2*sn+1);
            tmp = [sol.stats];
            tmp = [tmp.nsteps]+1;
            ...
            if ~isempty(tmpid)
                warning("ACSL : LSA includes zero rows : delete these rows : TODO");
                LSA_param.x(tmpid,:) = [];
            LSA_param.y(tmpid,:) = [];
            LSA_param.a(tmpid,:) = [];
            LSA_param.b(tmpid,:) = [];
            ...
            association_info = obj.UKFMapAssociation(PreXh(1:obj.n),PreMh(1:end), EndPoint{1,1}, measured.ranges,measured.angles);
            ...
            if isempty(find(obj.result.AssociationInfo.index ~= 0, 1))
