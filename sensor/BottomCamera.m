classdef BottomCamera < SENSOR_CLASS
    ...
    methods
        ...
        function result = do(obj,FireMap)
            ...
            tyome(:,:) = FireMap{1,1}.env.firemap.param.MapX <= round(obj.self.plant.state.p(1)) + obj.param{1,1}.Range(1,2) &...
                         FireMap{1,1}.env.firemap.param.MapX >= round(obj.self.plant.state.p(1)) + obj.param{1,1}.Range(1,1) &...
                         FireMap{1,1}.env.firemap.param.MapY <= round(obj.self.plant.state.p(2)) + obj.param{1,1}.Range(2,2) &...
                         FireMap{1,1}.env.firemap.param.MapY >= round(obj.self.plant.state.p(2)) + obj.param{1,1}.Range(2,1);
