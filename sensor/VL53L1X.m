classdef VL53L1X < SENSOR_CLASS
    ...
    methods
        function obj = VL53L1X(self,param)
            ...
            obj.KF.x_h = [];%状態推定値初期値
            obj.KF.P_h = 1;%共分散行列
            obj.KF.Q = 1*eye(2);%システムノイズの分散
            obj.KF.R = 3*eye(2);%観測ノイズの分散
            obj.KF.A = eye(2);
            obj.KF.b = [1;1];
            obj.KF.c = eye(2);
            obj.z_d = 0;
...
%                 obj.result.distance.esprT = sensor_distance(2);
                ...
                %% 実際の天井との距離に変換
                obj.result.distance.VL = (obj.KF.x_h(1))*cos(Roll)*cos(Pitch);
                obj.result.distance.teraranger = (obj.KF.x_h(2))*cos(Roll)*cos(Yaw);
                obj.result.switch = 0;
