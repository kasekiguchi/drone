classdef RANGE_COVERAGE_BIRD_SIM < SENSOR_CLASS
    ...
    methods
        ...
        function result = do(obj,Param)
            ...
            sensor = obj.self.sensor.rpos_bird.result.neighbor; % センサで取ったtoriの位置
            N = Param{1};
            for i = N
                sensor_drone(:,i) = obj.drone(i).plant.state.p; % センサで取ったdroneの全部の位置
            end
...
%             id = obj.self.id;
            ...
            %% 鳥の位置を取得
            result.drone_pos = sensor_drone;
            result.bird_pos = sensor;
            obj.result = result;
        end
