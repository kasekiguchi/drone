...
FieldType = FieldSelect{6};
SponeSelect = {'Alignment','Random','SeparateAlignment','SeparateRandom'};
SponeType = SponeSelect{2};
...
%human save information
for i =1:N
    yasui.save.agent{i}.state=[];
end
human_wark =zeros(N,1);
yasui.save.imp = cell(1);
yasui.dt = dt;
respone_flag=zeros(1,N);
yasui.main_roop_count = 1;
...
for i = 1:N
    ...
    if fExp
        ...
        %         agent(i) = Drone(Model_human_6para(i,N,dt,'plant',initial(i))); % unit quaternionのプラントモデル : for sim
        if i<=Nh
        agent(i) = Drone(Model_human_accelaration(i,N,dt,'plant',initial(i))); % 人の動作モデル : for sim
        else
        agent(i) = Drone(Mode_obstacles(i,N,dt,'plant',initial(i))); % 人の動作モデル : for sim
        end        
    end
    %% model
    % set control model
    ...
    %     agent(i).set_model(Model_human_6para(i,N,dt,'model',initial(i))); % 人質点モデル
    if i<=Nh
    agent(i).set_model(Model_human_accelaration(i,N,dt,'model',initial(i))); % 人加速度入力
    else
    agent(i).set_model(Mode_obstacles(i,N,dt,'model',initial(i))); % 人加速度入力
    end
    ...
    %     agent(i).set_property("env",Env_human_load(i)); % ノード設定用
    agent(i).set_property("env",Env_human_slope(i,FieldType)); % スロープ設定用
    ...
    %         agent(i).set_property("reference",Reference_human("ref",{agent(i).env.huma_load.param,i}));%ノード入力のやつ
    if i<=Nh
    agent(i).set_property("reference",Reference_slope("ref",{agent(i).env.huma_load.param,i,N}));%スロープ使うやつ
    else
    agent(i).set_property("reference",Reference_obstacle("ref",{agent(i).env.huma_load.param,i,N}));%スロープ使うやつ
    end
    %     if fExp == 1
    %         if i ==1
    %             agent(i).set_property("reference",Reference_Time_Varying("Case_study_trajectory",[-1;0;1])); % ハート形[x;y;z]永久
    ...
    %     agent(i).set_property("controller",struct("type","DirectController", "name","direct","param",[1;1]));% 次時刻に入力の位置に移動するモデル用：目標位置を直接入力とする
    ...
    agent(i).set_property("controller",Controller_human(dt));%離散コントローラと一緒
    %%
...
%% 初期設定とか
AllSateP = arrayfun(@(i) agent(i).plant.state.p(1:2),1:N,'UniformOutput',false);
figure(10)

for i=1:N
    R_name = agent(i).reference.name{1};
    if strcmp('slope',R_name)
        SponepointSetting = cov_SponepointSetting(agent(i).env.huma_load.param,i,N);
        sponepoint = Cov_randspone(SponepointSetting(1),SponepointSetting(2),SponepointSetting(3),SponepointSetting(4),agent(i),SponeType,AllSateP);
        agent(i).plant.state.p(1:2)=[sponepoint.x;sponepoint.y];%人間を通路の端に配置
        if strcmp('straight',agent(i).env.huma_load.param.txt)
            if i==1
                agent(i).plant.state.p(1:2) = [3;1];
            elseif i==3
                agent(i).plant.state.p(1:2) = [3;1.2];
            elseif i==2
                agent(i).plant.state.p(1:2) = [3.1;7];
            elseif i==4
    ...
    AllSateP = arrayfun(@(i) agent(i).plant.state.p(1:2),1:N,'UniformOutput',false);
    plot(agent(i).plant.state.p(1),agent(i).plant.state.p(2),'r:*')
    hold on
end
...
FH  = figure('position',[0 0 0.000000001 0.000000001],'menubar','none');
%キー入力検知
...
while round(time.t,5)<=te
    ...
    for i = 1:N
        ...
        param(i).reference.human={agent(i).sensor.result,agent(i).env,N,i,yasui,agent};
        param(i).reference.slope={agent(i).sensor.result,agent(i).env,{N,Nh,Nob},i,yasui,agent};
        param(i).reference.obstacle={agent(i).sensor.result,agent(i).env,{N,Nh,Nob},i,yasui,agent};
        ...
        agent(i).do_controller({time.t,agent(i).reference.result.state.u});
...
%         agent(i).do_plant(model_param);
    ...
    yasui.dencity = Cov_CalclateDensity(agent,[0 6;0 8;4 8;4 6],yasui);
    [yasui,respone_flag] = yasui_SAVE_state_human(agent,yasui,respone_flag,Nh,Nob);
    ...
    parfor i=1:Nh
        ...
        if ResponeJudgement
            ...
            agent(i).plant.state.p(1:2)=responepoint;
            human_wark(i) = 0;
            respone_flag(i) = 1;
            agent(i).reference.slope.random = randn;
        end
    end
    %%人口密度算出用関数
    %個体位置と空間の四隅を記述
...
while 1
    T = toc;
    if strcmp(get(FH,'currentcharacter'),'y')||T>=100
        cd 'C:\Users'
        TFa = exist('Tyasui','dir');
        TFb = exist('owner','dir');
        close(FH)
        if TFa==7&&TFb==0
            cd 'C:\Users\Tyasui\OneDrive - tcu.ac.jp\work2020\work\video\human\data'
            savecount = dir('*.mat');
            text = [num2str(length(savecount))];
            save(text,'yasui');
            break
        elseif TFb==7&&TFa==0
            cd 'C:\Users\owner\OneDrive - tcu.ac.jp\work2020\work\video\human\data'
            savecount = dir('*.mat');
            text = [num2str(length(savecount))];
            save(text,'yasui');
            break
    ...
    elseif strcmp(get(FH,'currentcharacter'),'n')
        disp('Not Save')
        break
    end
