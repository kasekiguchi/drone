...
if time.t == 9
    time.t;
        ...
        for i = 1:N
            ...
            param(i).reference.point = {FH, [2; 2; 1], time.t};
...
%             if time.t <= 10
%                 agent(i).input = agent(i).input - [0.5;0.01;0;0]; % 定常外乱
%             else
%                 agent(i).input = agent(i).input - [0.8;0.01;0;0];
%             end
%                 agent(i).input = agent(i).input - [0+(5/10);0.01;0;0];
...
logger.plot({1,"p1","pr"},"time",[0 50],"fig_num",1);
legend("off")
xlabel('Time[s]','FontSize',25)
ylabel('Position[m]','FontSize',25)
logger.plot({1,"p2","pr"},"fig_num",2);
legend("off")
xlabel('Time[s]','FontSize',25)
ylabel('Position[m]','FontSize',25)
logger.plot({1,"p3","pr"},"time",[0 50],"fig_num",3);
legend("off")
xlabel('Time[s]','FontSize',25)
ylabel('Position[m]','FontSize',25)
