...
%             agent(i).do_reference(param(i).reference.list);
            ...
            %HL method->q, w　0.1秒ごとにモデルの姿勢角等が必要なため、それぞれで回して（入力をそれぞれで得る）姿勢角も取得する
                ...
                for HLi = 1:Params.H-1
                    ...
                    % reference
                    param(i).reference.covering = [];
                    param(i).reference.point = {FH, [rx;ry;rz], HLt};  % 目標値[x, y, z]
                    param(i).reference.timeVarying = {time};        %  時間を回す
                    param(i).reference.tvLoad = {time};
                    param(i).reference.wall = {1};
                    param(i).reference.agreement = {logger, N, time.t};
                    for j = 1:length(agent(i).reference.name)
...
for i= 1:size_best
    ...
    HLX(:, i) = data.HLx{1, i}(:, 2);
    HLInput(:, i) = data.HLinput{1, i}(:, 2);
 end
...
hold off;
% HL controller 
figure(12); plot(logger.Data.t(1:size_best, :), HLX(1:3, :)); xlabel("Time [s]"); ylabel("HL controller reference"); legend("p1","p2","p3"); set(gca,'FontSize',Fontsize);
figure(13); plot(logger.Data.t(1:size_best, :), HLInput); xlabel("Time [s]"); ylabel("HL controller input"); legend("u1","u2","u3","u4"); set(gca,'FontSize',Fontsize);
%% animation
