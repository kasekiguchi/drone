...
%             obj.B = obj.param.model.est.B; %出力ベクトルB, 複素数空間上での値
%             obj.f = @(x) [x;1];%観測量

            % p , q, v, w
                %~ ホライズン×サンプル数の配列初期化
                %~ 状態数×ホライズン×サンプル数に次元変換
%             obj.state.state_data = [obj.state.p_data;obj.state.q_data;obj.state.v_data;obj.state.w_data;obj.state.constant];
