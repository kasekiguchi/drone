...
%     var = zeros(Params.total_size, Params.H);
...
%             Aeq = double(ones(1,12));
% %             beq = @(x) Constraints2(x, Params);
%             beq = 10;
%             A = [];
%             B = [];
%             lb = [];
...
%             [var, fval, exitflag, output, lambda, grad, hessian] = fmincon(fun, x0, A, B,  Aeq, beq, lb, ub,nonlcon, problem);
