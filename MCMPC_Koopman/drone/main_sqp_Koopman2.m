...
%     Params.Weight.QW = diag([10; 10; 10; 0.01; 0.01; 100.0]);  % 姿勢角、角速度
    ...
    % 円旋回(重みの設定)
    Params.Weight.P = diag([1.0; 1.0; 10.0]);    % 座標   1000 10
    Params.Weight.V = diag([1.0; 1.0; 1.0]);    % 速度
    Params.Weight.R = diag([1.0,; 1.0; 1.0; 1.0]); % 入力
    Params.Weight.RP = diag([0; 0; 0; 0]);  % 1ステップ前の入力との差    0*(無効化)
    Params.Weight.QW = diag([4000; 4000; 3000; 1; 1; 1]);  % 姿勢角、角速度

    Params.Weight.Pf = diag([1; 1; 10]);
    Params.Weight.QWf = diag([4000; 4000; 3000; 1; 1; 1]); %姿勢角、角速度終端
    %% 
