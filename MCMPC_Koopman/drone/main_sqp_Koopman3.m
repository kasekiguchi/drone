...
%     Params.Weight.QW = diag([10; 10; 10; 0.01; 0.01; 100.0]);  % 姿勢角、角速度
    ...
    Params.Weight.QW = diag([2500;2000;1000; 1; 1; 1]);  % 姿勢角、角速度

    Params.Weight.Pf = diag([1; 5; 1]);
    Params.Weight.QWf = diag([5000; 3000; 1200; 1; 1; 1]); %姿勢角、角速度終端
    %% 
...
logger.plot({1,"p","e"},{1,"v","e"},{1,"q","e"},{1,"w","e"},{1,"input",""},{1, "p1-p2", "e"}, "fig_num",1,"row_col",[2,3]);
% figure
% plot(data.exitflag)
%% Difference of Pos
% figure(7);
