...
%   詳細説明をここに記述
    ...
    Controller_param.particle_num = 3000;  %サンプル数
    Controller_param.Initsigma = 0.0001;     %初期標準偏差
%     Controller_param.ref_input = [0.5884 * 9.81 / 4 0.5884 * 9.81 / 4 0.4 * 9.81 / 4 0.5884 * 9.81 / 4]'; %初期入力
%     Controller_param.ref_input = [1.4 1.55 1.4 1.55]';
    Controller_param.ref_input = [1.443 1.443 1.443 1.443]';
    Controller_param.model = load('drone\MCMPC_Koopman\drone\koopman_data\EstimationResult_12state_7_20_simulation_circle.mat','est');
...
%     obj.param.QW = diag([10; 10; 10; 0.01; 0.01; 100.0]);  % 姿勢角、角速度
    ...
    %% 重みの設定
    Controller_param.P = diag([1000.0; 15000.0; 6000.0]);    % 座標   1000 1000 100
    ...
    Controller_param.Q = diag([4000; 4000; 3000]);                % 姿勢角
